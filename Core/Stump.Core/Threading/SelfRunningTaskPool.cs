...
namespace Stump.Core.Threading
{
    ...
    public class SelfRunningTaskPool : IContextHandler
    {
        ...
        {
            if (message.ToString() != "Stump.Server.WorldServer.Core.IPC.IPCAccessor+<>c__DisplayClass80_0.<BuildMessage>b__0" && message.ToString() != "Stump.Core.Threading.SelfRunningTaskPool+<>c__DisplayClass37_0.<AddTimer>b__0")
            logger.Info("message in queue {0}", message);
        ...
        public bool ExecuteInContext(Action action)
        {
            if (IsInContext)
            {
                logger.Info("ExecuteInContext Action Method: {0}", action.Method.ToString());
                action();
                return true;
            }
        ...
        public void Debug()
        {
            logger.Info("---------DEBUG PART 1 ---------------");
            foreach (var test in m_messageQueue)
            {
                test.ToString();
            }
            logger.Info("---------DEBUG PART 2 ---------------");
            IMessage msg;
            while (m_messageQueue.TryDequeue(out msg))
                logger.Info("DEBUG message in queue {0}", msg);
        ...
        protected void ProcessCallback(object state)
        {
            if (!IsRunning)
            {
                logger.Info($"taskpoll NOT RUNING {this} exit callback since it's disposed");
                return;
            }
            ...
            {
                logger.Info($"taskpoll THREAD {this} exit callback since it's disposed");
                return;
            }
            ...
            try
            {
                ...
                while (m_messageQueue.TryDequeue(out msg))
                {
                    ...
                    try
                    {
                        if (msg.ToString() != "Stump.Server.WorldServer.Core.IPC.IPCAccessor+<>c__DisplayClass80_0.<BuildMessage>b__0" && msg.ToString() != "Stump.Core.Threading.SelfRunningTaskPool+<>c__DisplayClass37_0.<AddTimer>b__0")
                            logger.Info("Execute msg {0}", msg);
                        msg.Execute();
                    }
                    catch (Exception ex)
                    {
                    ...
                    {
                        if (msg.ToString() != "Stump.Server.WorldServer.Core.IPC.IPCAccessor+<>c__DisplayClass80_0.<BuildMessage>b__0" && msg.ToString() != "Stump.Core.Threading.SelfRunningTaskPool+<>c__DisplayClass37_0.<AddTimer>b__0")
                            logger.Info("NOTRUNNING??!! msg {0}", msg);
                        return;
                    }
                }
                //logger.Info("foreach with m_pausedTimers:{0}", m_pausedTimers.Count());
                foreach (var timer in m_pausedTimers.Where(timer => timer.Enabled))
                {
                    m_timers.Push(timer);
                }
                //logger.Info("While peek:{0}", m_timers.Count());
            ...
            {
                //logger.Info("Finally Execute message in Taskpoll");
                // get the end time
...
#endif
                    //logger.Info("Finally End Execute message in Taskpoll");
                }
                if (IsRunning)
                {
                    // re-register the Update-callback
                    //logger.Info("re-register  Taskpoll");
                    m_updateTask = Task.Factory.StartNewDelayed((int) callbackTimeout, ProcessCallback, this);
                }
                else
                {
                    logger.Info("re-register NOTRUNNING! Taskpoll");
                    m_stoppedAsync.Set();
                }
            }
        }
