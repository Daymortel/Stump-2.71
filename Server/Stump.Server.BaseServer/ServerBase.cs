...
namespace Stump.Server.BaseServer
{
    ...
    public abstract class ServerBase
    {
        ...
        public TimeSpan DownTime
        {
            get
            {
                TimeSpan AutoShut = TimeSpan.FromMinutes(AutomaticShutdownTimer);

                return ((DateTime.Now - StartTime) - AutoShut);
            }
        }
        ...
        public virtual void Initialize()
        {
            ...
            if (Settings.InactivityDisconnectionTime.HasValue)
                IOTaskPool.CallPeriodically(Settings.InactivityDisconnectionTime.Value * 1000, DisconnectAfkClient);
        ...
        {
            List<BaseClient> clientsAFK = ClientManager.FindAll(client => DateTime.Now.Subtract(client.LastActivity).TotalMinutes >= Settings.InactivityDisconnectionTime).ToList();

            foreach (var client in clientsAFK)
            {
                logger.Warn($"The client IP {client.IP} was disconnected due to inactivity of {DateTime.Now.Subtract(client.LastActivity).TotalMinutes}/{Settings.InactivityDisconnectionTime} minutes on ServerBase.");
                client.Disconnect();
            }
        }
