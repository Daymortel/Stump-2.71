...
namespace Stump.Server.AuthServer.Managers
{
    ...
    public class WorldServerManager : DataManager<WorldServerManager>
    {
        ...
        {
            //AuthServer.Clients.Where(x => x != null).Where<AuthClient>
            ClientManager.Instance.FindAll<AuthClient>(entry => entry.LookingOfServers).ForEach(entry => ConnectionHandler.SendServerStatusUpdateMessage(entry, server));
        ...
        {
            //AuthServer.Clients.Where(x => x != null).Where<AuthClient>
            ClientManager.Instance.FindAll<AuthClient>(entry => entry.LookingOfServers).ToList().ForEach(entry => ConnectionHandler.SendServerStatusUpdateMessage(entry, server));
        ...
        public WorldServer RequestConnection(IPCClient client, WorldServerData world)
        {
            ...
            {
                logger.Error("Server <Id : {0}> ({1}) Try to connect self on the authenfication server but its ip is not allowed (see WorldServerManager.AllowedServerIps)", world.Id, client.Address);
        ...
        public GameServerInformations[] GetServersInformationArray(AuthClient client) => m_realmlist.Values.Where(x => client.UserGroup.CanAccessWorld(x)).Select(world => GetServerInformation(client, world)).ToArray();

        public GameServerInformations GetServerInformation(AuthClient client, WorldServer world)
        {
            sbyte charactersCount = client.Account.GetCharactersCountByWorld;
            sbyte charactersCountServer = 0;
            sbyte charactersCountLimitAccount = 0;

            //TODO
            if (charactersCount > 0) //Correction to display the list of servers in version 2.61.10.19 by Kenshin
            {
                charactersCountServer = charactersCount;
                charactersCountLimitAccount = (sbyte)(charactersCount > 5 ? charactersCount : 5);
            }
            else
            {
                charactersCountServer = 1;
                charactersCountLimitAccount = 5;
            }

            return new GameServerInformations(
                isMonoAccount: false,
                isSelectable: world.ServerSelectable,
                id: (ushort)world.Id,
                type: 1,
                status: (sbyte)world.Status,
                completion: (sbyte)world.Completion,
                charactersCount: charactersCountServer,
                charactersSlots: charactersCountLimitAccount,
                date: DateTime.Now.GetUnixTimeStampLong());
        }
