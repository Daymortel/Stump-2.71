...
namespace Stump.Server.AuthServer.Managers
{
    public class AccountManager : DataManager<AccountManager>
    {
        ...
        {
            base.Initialize();
            //IEnumerable<Vote> transactionsData = Database.Fetch<Vote>("SELECT * FROM votes", new object());
            //IEnumerable<Account> accounts = Database.Fetch<Account>("Select * FROM accounts", new object());

            //foreach(Account account in accounts)
            //{
            //    IEnumerable<Vote> transactionsAccount = transactionsData.Where(x => x.AcctID == account.Id);
            //    foreach(Vote transaction in transactionsAccount)
            //    {
            //        account.Tokens += 10;
            //    }
        ...
        void TimerTick()
        {
            var toRemove = (from keyPair in m_accountsCache.ToList() where keyPair.Value.Item1 <= DateTime.Now select keyPair).ToList();
        ...
        {
            return Database.Query<Account, WorldCharacter, Account>(new AccountRelator().Map, string.Format(AccountRelator.FindAccountById, id)).SingleOrDefault();
        }

        public Account FindAccountByLogin(string login)
        {
            return Database.Query<Account, WorldCharacter, Account>(new AccountRelator().Map, AccountRelator.FindAccountByLogin, login).SingleOrDefault();
        }

        public Account FindAccountByNickname(string nickname)
        {
            return Database.Query<Account, WorldCharacter, Account>(new AccountRelator().Map, AccountRelator.FindAccountByNickname, nickname).SingleOrDefault();
        }

        public Account FindAccountByCharacterId(int characterId)
        {
            return Database.Query<Account, WorldCharacter, Account>(new AccountRelator().Map, string.Format(AccountRelator.FindAccountByCharacterId, characterId)).SingleOrDefault();
        }
        ...
        public bool CreateAccount(Account account)
        {
            try
            {
                ...
                AuthServer.Instance.IOTaskPool.ExecuteInContext(() =>
                {
        ...
        public bool CreateAccountCharacter(Account account, WorldServer world, int characterId)
        {
            try
            {
            ...
            {
                Console.WriteLine($"Error during account update: {ex.Message}");
                return false;
            }
        }


        public bool AddAccountCharacter(Account account, WorldServer world, int characterId) => CreateAccountCharacter(account, world, characterId);
        ...
        public void DisconnectClientsUsingAccount(Account account, AuthClient except, Action<bool> callback, Action errorCallBack)
        {
            ...
            {
                server.IPCClient.SendRequest<DisconnectedClientMessage>(new DisconnectClientMessage(account.Id), msg => callback(msg.Disconnected), msg => errorCallBack());
            }
            else
            {
                callback(false);
