...
namespace Stump.Server.AuthServer.IPC
{
    public class IPCOperations
    {
        ...
        {
            Client.TaskPool.ExecuteInContext(() =>
            {
                if (WorldServer.CharsCount == message.CharsCount && (message.Status == ServerStatusEnum.STATUS_UNKNOWN || (message.Status == ServerStatusEnum.ONLINE && WorldServer.Status == ServerStatusEnum.ONLINE)))
                {
                    Client.ReplyRequest(new CommonOKMessage(), message);
                    return;
                }
                ...
                message.Status = WorldServer.Status == ServerStatusEnum.SAVING && message.Status != ServerStatusEnum.FINISH_SAVE ? ServerStatusEnum.SAVING : message.Status;
                if (message.Status == ServerStatusEnum.ONLINE)
                {
                    //dont give a fuck
                    if (WorldServer.CharsCount >= WorldServer.CharCapacity && WorldServer.Status == ServerStatusEnum.ONLINE)
                    {
                        //WorldServer.Status = ServerStatusEnum.FULL;//message.Status;
                        WorldServerManager.Instance.ChangeWorldState(WorldServer, ServerStatusEnum.FULL);
                    }
                    else
                    {
                        //WorldServer.Status = message.Status;
                        WorldServerManager.Instance.ChangeWorldState(WorldServer, ServerStatusEnum.ONLINE);
                    }
                }
                else if (message.Status != ServerStatusEnum.STATUS_UNKNOWN)
                {
                    //WorldServer.Status = message.Status;
                    WorldServerManager.Instance.ChangeWorldState(WorldServer, message.Status);
                }


                Client.TaskPool.EnsureContext();
        ...
        private void Handle(UpdateAccountMessage message)
        {
            Client.TaskPool.ExecuteInContext(() =>
            {
                ...
                account.RecievedRewards = message.Account.RecievedRewards;
        ...
        private void Handle(UpdateVipAccountMessage message)
        {
            Client.TaskPool.ExecuteInContext(() =>
            {
                var account = AccountManager.FindAccountById(message.Account.Id);
                ...
                account.Role = message.UserGroupId;
                account.SubscriptionEnd = message.SubscriptionEndDate;
                account.GoldSubscriptionEnd = message.GoldSubscriptionEndDate;
        ...
        private void Handle(AccountLastDaysMessage message)
        {
            Client.TaskPool.ExecuteInContext(() =>
            {
                var account = AccountManager.FindAccountById(message.Account.Id);
                ...
                account.LastDaysConnection = message.LastDaysConnection;
        ...
        private void Handle(UpdateTokensMessage message)
        {
            Client.TaskPool.ExecuteInContext(() =>
            {
        ...
        public void Dispose()
        {
            ...
            //Client.TaskPool.ExecuteInContext(() => Database.CloseSharedConnection());  
            m_handlers.Clear();
        }
    }
}