...
namespace Stump.Server.WorldServer.AI.Fights.Spells
{
    public class SpellSelector
    {
        ...
        private IEnumerable<TargetCell> ExpandZone(Cell center, SpellShapeEnum shape, int minRange, int maxRange)
        {
            switch (shape)
            {
                case SpellShapeEnum.X:
                    return new CrossSet(new MapPoint(center), (short)maxRange, (short)minRange).
        ...
        public int MaxConsecutiveSpellCast(Spell spell, int ap)
        {
            ...
            if (spell.CurrentSpellLevel.ApCost == 0 && ap == 0 && spell.CurrentSpellLevel.MinCastInterval == 0)
                return 0;
            ...
            if(spell.CurrentSpellLevel.ApCost > 0)
                max = (int)(ap / spell.CurrentSpellLevel.ApCost);
            ...
            if (spell.CurrentSpellLevel.MaxCastPerTarget > 0 && max > spell.CurrentSpellLevel.MaxCastPerTarget)
                max = (int)spell.CurrentSpellLevel.MaxCastPerTarget;

            if (spell.CurrentSpellLevel.MaxCastPerTurn > 0 && max > spell.CurrentSpellLevel.MaxCastPerTurn)
