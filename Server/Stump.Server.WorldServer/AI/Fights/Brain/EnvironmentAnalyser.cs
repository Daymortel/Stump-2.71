    ...
    public class EnvironmentAnalyser
    {
        ...
        public Cell GetCellToCastSpell(TargetCell target, Spell spell, bool LoS, bool nearFirst = true)
        {
            ...
            if (spell.CurrentSpellLevel.CastInLine || spell.CurrentSpellLevel.CastInDiagonal)
                castRange = new CrossSet(target.Point, (short)Fighter.GetSpellRange(spell.CurrentSpellLevel),
                    (short)(spell.CurrentSpellLevel.MinRange != 0 ? (int)spell.CurrentSpellLevel.MinRange : CellInformationProvider.IsCellWalkable(target.Cell.Id) ? 0 : 1))
                {
        ...
        {
            return GetCellsWithLoS(target, new LozengeSet(Fighter.Position.Point, (int)Fighter.MP));
        }
        ...
        public IEnumerable<Cell> GetCellsWithLoS(Cell target, Set searchZone)
        {
            foreach (var cell in searchZone.EnumerateValidPoints())
            {
                ...
                {
                    if (Fight.CanBeSeen(cell, MapPoint.GetPoint(target), true))
        ...
        {
            return m_moveZone ?? (m_moveZone = Pathfinding.FindReachableCells(Fighter.Position.Point, (int)Fighter.MP));
        }
        ...
        public bool TryToReach(MapPoint point, out Path path)
        {
            ...
            path = Pathfinding.FindPath(Fighter.Position.Point, point, false, (int)Fighter.MP);
        ...
        {
            return GetMovementCells((int)Fighter.MP);
        }
        ...
        public Cell[] GetMovementCells(int mp)
        {
            ...
            return circle.GetCells(Fighter.Cell, Fighter.Cell, Fight.Map);
        }
