...
{
    [BrainIdentifier((int)MonsterIdEnum.SIMBADAS_4814)]
    public class Simbadas : Brain
    {
        public Simbadas(AIFighter fighter) : base(fighter)
        {
        ...
        {
            Fighter.CastAutoSpell(new Spell((int)SpellIdEnum.CALL_OF_HUNGER_8536, 1), Fighter.Cell);
            ...
            foreach (var spell in Fighter.Spells.Values)
            {
                ...
                if (target != null && spell != null)
                {
                    selector.AddChild(new PrioritySelector(new Decorator(ctx => Fighter.CanCastSpell(spell, target.Cell) == SpellCastResult.OK, new Sequence(
                        new SpellCastAction(Fighter, spell, target.Cell, true), new Decorator(new MoveNearTo(Fighter, target)))),
