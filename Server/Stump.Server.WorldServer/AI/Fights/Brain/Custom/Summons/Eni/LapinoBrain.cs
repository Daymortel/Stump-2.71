...
namespace Stump.Server.WorldServer.AI.Fights.Brain.Custom.Summons.Eni
{
    [BrainIdentifier((int)MonsterIdEnum.LAPINO_39)]
    [BrainIdentifier((int)MonsterIdEnum.LAPINO_5904)]
    [BrainIdentifier((int)MonsterIdEnum.LAPINO_PROTECTEUR_5905)]
    class Bunny : Brain
    {
        public Bunny(AIFighter fighter) : base(fighter)
        {
        ...
        void OnFighterAdded(FightTeam team, FightActor fighter)
        {
            ...
            if (fighter is SummonedMonster && (fighter as SummonedMonster).Monster.MonsterId == (int)MonsterIdEnum.LAPINO_39 || (fighter as SummonedMonster).Monster.MonsterId == (int)MonsterIdEnum.LAPINO_5904)
            {
                fighter.CastAutoSpell(new Spell((int)SpellIdEnum.STIMULATING_WORD_13173, (short)fighter.Level), fighter.Cell);
            }

            if (fighter is SummonedMonster && (fighter as SummonedMonster).Monster.MonsterId == (int)MonsterIdEnum.LAPINO_PROTECTEUR_5905)
            {
                fighter.CastAutoSpell(new Spell((int)SpellIdEnum.STIMULATING_WORD_7509, (short)fighter.Level), fighter.Cell);
            }
        }
        ...
        public void OnBeforeDead(FightActor fighter, FightActor killedBy)
        {
            ...
            if (fighter is SummonedMonster && (fighter as SummonedMonster).Monster.MonsterId == (int)MonsterIdEnum.LAPINO_39 || (fighter as SummonedMonster).Monster.MonsterId == (int)MonsterIdEnum.LAPINO_5904)
            {
                (fighter as SummonedMonster).Summoner.CastAutoSpell(new Spell((int)SpellIdEnum.FRIENDSHIP_WORD_13197, 1), fighter.Cell);
            }
            if (fighter is SummonedMonster && (fighter as SummonedMonster).Monster.MonsterId == (int)MonsterIdEnum.LAPINO_PROTECTEUR_5905)
            {
                (fighter as SummonedMonster).Summoner.CastAutoSpell(new Spell((int)SpellIdEnum.AFFECTIONATE_WORD_13236, 1), fighter.Cell);
            }
        }
        ...
        public override void Play()
        {
            foreach (var spell in Fighter.Spells.Values)
            {
                ...
                if (Fighter.Id == (int)MonsterIdEnum.LAPINO_PROTECTEUR_5905)
                {
