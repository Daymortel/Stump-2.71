    ...
    public class JailCommand : TargetCommand
    {
        ...
        public override void Execute(TriggerBase trigger)
        {
            ...
            foreach (var target in GetTargets(trigger))
            {
                ...
                #region // ----------------- Sistema de Logs MongoDB Jail by: Kenshin ---------------- //
                try
                {
                    var document = new BsonDocument
                                {
                                    ...
                                    { "Target_TempJailEnd",  message.BanEndDate },
                                    { "Date", DateTime.Now.ToString(CultureInfo.CreateSpecificCulture("pt-BR")) }
                                };

                    MongoLogger.Instance.Insert("Staff_ManagementJail", document);
                }
                catch (Exception e)
                {
            ...
            foreach (var target in GetTargets(trigger))
            {
                ...
                switch (target.Account.Lang)
                {
                    case "fr":
                        ...
                        break;
                    case "es":
                        target.SendServerMessage("Usted ha sido liberado de la prisión.", Color.Red);
                        break;
                    case "en":
                        target.SendServerMessage("You have been released from prison.", Color.Red);
                        break;
                    default:
                        target.SendServerMessage("Você foi libertado da prisão.", Color.Red);
                        break;
                }
            }
        }
