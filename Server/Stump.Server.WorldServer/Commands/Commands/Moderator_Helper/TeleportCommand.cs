...
namespace Stump.Server.WorldServer.Commands.Commands.Moderator
{
    ...
    public class GoNameCommand : CommandBase
    {
        ...
        public override void Execute(TriggerBase trigger)
        {
            ...
            #region // ----------------- Sistema de Logs MongoDB Teleport by: Kenshin ---------------- //
            try
            {
                var document = new BsonDocument
                                {
                                    ...
                                    { "Staff_MapID", source.Character.Map.Id },
                                    { "Target_ID", (trigger as GameTrigger).Character.Id },
                                    { "Target_Name", (trigger as GameTrigger).Character.NameClean },
                                    { "Target_IP", (trigger as GameTrigger).Character.Client.IP },
                                    { "Target_MapID",  (trigger as GameTrigger).Character.Map.Id },
                                    { "Function", "GoName" },
                                    { "Date", DateTime.Now.ToString(CultureInfo.CreateSpecificCulture("pt-BR")) }
                                };

                MongoLogger.Instance.Insert("Staff_ManagementTeleport", document);
            }
            catch (Exception e)
            {
    ...
    public class NameGoCommand : TargetCommand
    {
        ...
        public override void Execute(TriggerBase trigger)
        {
            foreach (var target in GetTargets(trigger))
            {
                ...
                target.Area.ExecuteInContext(() => target1.Teleport(to.Position));
