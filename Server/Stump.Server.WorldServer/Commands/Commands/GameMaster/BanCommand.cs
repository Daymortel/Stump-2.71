        ...
        public override void Execute(TriggerBase trigger)
        {
            ...
            foreach (var target in GetTargets(trigger))
            {
                ...
                if (trigger.IsArgumentDefined("time"))
                {
                    ...
                    if (time > 60 * 24 && trigger.UserRole == RoleEnum.GameMaster)
                        // max ban time for padawan == 24h
                        time = 60 * 24;
                    message.BanEndDate = DateTime.Now + TimeSpan.FromMinutes(time);
                }
                else if (trigger.IsArgumentDefined("life") && trigger.UserRole != RoleEnum.GameMaster)
    ...
    public class UnBanCommand : CommandBase
    {
        ...
        public override void Execute(TriggerBase trigger)
        {
            ...
            else
            {
                ...
                switch (player.Account.Lang)
                {
                    case "fr":
                        player.SendServerMessage("Vous avez été libéré de prison.", Color.Red);
                        break;
                    case "es":
                        player.SendServerMessage("Usted ha sido liberado de la prisión.", Color.Red);
                        break;
                    case "en":
                        player.SendServerMessage("You have been released from prison.", Color.Red);
                        break;
                    default:
                        player.SendServerMessage("Você foi libertado da prisão.", Color.Red);
                        break;
                }
            }
        }
