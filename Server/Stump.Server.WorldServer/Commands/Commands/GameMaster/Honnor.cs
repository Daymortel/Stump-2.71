using Stump.DofusProtocol.Enums;
using Stump.Server.BaseServer.Commands;
using Stump.Server.WorldServer.Commands.Commands.Patterns;
using Stump.Server.WorldServer.Commands.Trigger;
using Stump.Server.WorldServer.Core.Network;
using System.Drawing;

namespace Stump.Server.WorldServer.Commands.Commands.GameMaster
{

    public class HonorCommand : SubCommandContainer
    {
        public HonorCommand()
        {
            Aliases = new[] { "honor" };
            RequiredRole = RoleEnum.GameMaster;
            Description = "Manage your honor.";
        }
    }

    public class HonorAddCommand : TargetSubCommand
    {
        public HonorAddCommand()
        {
            Aliases = new[] { "add" };
            RequiredRole = RoleEnum.GameMaster;
            ParentCommandType = typeof(HonorCommand);
            Description = "Add Honor to a target";
            AddParameter<ushort>("amount", "amount", "Amount of honor to add", 1);
            AddTargetParameter(true);
        }

        public override void Execute(TriggerBase trigger)
        {
            var source = trigger.GetSource() as WorldClient;

            foreach (var target in GetTargets(trigger))
            {
                if (source.Character.UserGroup.Role >= RoleEnum.Moderator_Helper && source.Character.UserGroup.Role <= RoleEnum.Administrator)
                {
                    if (source.Character.Id == target.Client.Character.Id)
                    {
                        target.AddHonor(trigger.Get<ushort>("amount"));
                        target.Map.Refresh(target);
                        trigger.Reply("Félicitations ! Vous avez gagné " + trigger.Get<ushort>("amount") + " points d'honneur !", Color.OrangeRed);
                    }
                }
                else
                {
                    if (source.Character.UserGroup.Role >= RoleEnum.Non_ADM)
                    {
                        target.AddHonor(trigger.Get<ushort>("amount"));
                        target.Map.Refresh(target);
                        trigger.Reply("Félicitations ! Vous avez gagné " + trigger.Get<ushort>("amount") + " points d'honneur !", Color.OrangeRed);
                    }
                }
            }
        }
    }
}
