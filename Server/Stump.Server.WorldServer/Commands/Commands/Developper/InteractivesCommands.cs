...
namespace Stump.Server.WorldServer.Commands.Commands.GameMaster
{
    ...
    public class InteractiveInfoCommand : SubCommand
    {
        public InteractiveInfoCommand()
        {
            Aliases = new[] { "info" };
            Description = "info interactives id";
            RequiredRole = RoleEnum.GameMaster_Padawan;
        ...
        public override void Execute(TriggerBase trigger)
        {
            ...
            var interactive = InteractiveManager.Instance.GetOneSpawn(x => x.Id == elementId);
            if (interactive == null)
                return;

            trigger.Reply($"Identifier : {interactive.Id} - Template Id: {interactive.TemplateId} - MapId: {interactive.MapId} - CellId: {interactive.CellId} - Animated - {interactive.Animated} - Element Id : {interactive.ElementId} - Customskills : {interactive.CustomSkills.Count}");
            foreach (var skill in interactive.CustomSkills.ToList())
            {
                if (skill is InteractiveCustomSkillRecord)
                {
                    var id = (skill as InteractiveCustomSkillRecord).Id;
                    var type =(skill as InteractiveCustomSkillRecord).Type;
                    var duration = (skill as InteractiveCustomSkillRecord).Duration;
                    var condition = (skill as InteractiveCustomSkillRecord).Condition;
                    var parameter0 = (skill as InteractiveCustomSkillRecord).Parameter0;
                    var parameter1 = (skill as InteractiveCustomSkillRecord).Parameter1;
                    var parameter2 = (skill as InteractiveCustomSkillRecord).Parameter2;
                    if ((skill as InteractiveCustomSkillRecord).Template != null) {
                        var templateID = (skill as InteractiveCustomSkillRecord).Template.Id;
                        var templateElementActionId = (skill as InteractiveCustomSkillRecord).Template.ElementActionId;
                        var templateInteractiveId = (skill as InteractiveCustomSkillRecord).Template.InteractiveId;
                        var templateName = (skill as InteractiveCustomSkillRecord).Template.Name;
                        if ((skill as InteractiveCustomSkillRecord).Template.Interactive != null)
                        {
                            var interactivename = (skill as InteractiveCustomSkillRecord).Template.Interactive.Name;
                            var interactivetype = (skill as InteractiveCustomSkillRecord).Template.Interactive.Type;
                            trigger.Reply($"Id : {id} - Type: {type} - Condition: {condition} - Duration: {duration} - parameter0: {parameter0} - parameter1: {parameter1} - parameter2 : {parameter2} - templateID : {templateID} - ElementActionId : {templateElementActionId} - InteractiveId : {templateInteractiveId} - templateName : {templateName} - interactivename(type) : {interactivename}({interactivetype})");
                        }else
                            trigger.Reply($"Id : {id} - Type: {type} - Condition: {condition} - Duration: {duration} - parameter0: {parameter0} - parameter1: {parameter1} - parameter2 : {parameter2} - templateID : {templateID} - ElementActionId : {templateElementActionId} - InteractiveId : {templateInteractiveId} - templateName : {templateName}");
                    }else
                        trigger.Reply($"Id : {id} - Type: {type} - Condition: {condition} - Duration: {duration} - parameter0: {parameter0} - parameter1: {parameter1} - parameter2 : {parameter2}");
                }else
                    trigger.Reply($"Id : {skill.Id} indisponivel!");
            }
