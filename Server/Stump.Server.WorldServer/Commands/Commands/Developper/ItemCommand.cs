...
namespace Stump.Server.WorldServer.Commands.Commands.Players
{
    ...
    {
        //private int[] itens_block = { 30307, 30308, 30309, 30360, 30361, 30362, 30317, 30322 };
        ...
        public override void Execute(TriggerBase trigger)
        {
            ...
            int AmountInfo = trigger.Get<int>("amount");

            foreach (var target in GetTargets(trigger))
            {

                //if (character.Account.UserGroupId >= 4 && character.Account.UserGroupId <= 7 && target.Account.UserGroupId <= 3 && character.WorldAccount.CountItensStaff == 3 && character.WorldAccount.LastCountItenStaff.Date != DateTime.Now.Date)
                //{
                //    character.WorldAccount.CountItensStaff = 0;
                //}

                if (trigger.UserRole == RoleEnum.Developer)
                {
                    var item = ItemManager.Instance.CreatePlayerItem(target, itemTemplate, trigger.Get<int>("amount"), trigger.IsArgumentDefined("max"));
                    ...
                    #region Messege Infor
                    if (character != target)
                    {
                        character.SendInformationMessage(TextInformationTypeEnum.TEXT_INFORMATION_MESSAGE, 961, item.Template.Id, item.Guid, item.Stack, target.Namedefault); //Agregaste %3 x {item,%1,%2} al inventario de %4.
                        target.SendInformationMessage(TextInformationTypeEnum.TEXT_INFORMATION_MESSAGE, 962, item.Template.Id, item.Guid, item.Stack, character.Namedefault); //El personal de Charly %4 agregó %3 x {item,%1,%2} a tu inventario.
                    }
                    else
                    {
                        character.SendInformationMessage(TextInformationTypeEnum.TEXT_INFORMATION_MESSAGE, 963, item.Template.Id, item.Guid, item.Stack); //Ha agregado %3 x {item,%1,%2} a su inventario.
                    }
                    #endregion
                }
                else if (trigger.UserRole >= RoleEnum.GameMaster_Padawan && trigger.UserRole <= RoleEnum.Administrator && target.Account.UserGroupId >= 5)
                {
                    var item = ItemManager.Instance.CreatePlayerItem(target, itemTemplate, trigger.Get<int>("amount"), trigger.IsArgumentDefined("max"));
                ...
                //else if (trigger.UserRole >= RoleEnum.GameMaster_Padawan && trigger.UserRole <= RoleEnum.Administrator && target.Account.UserGroupId <= 3 && character.WorldAccount.CountItensStaff < 3 && !itens_block.Contains(itemTemplate.Id))
                //{
                //    var item = ItemManager.Instance.CreatePlayerItem(target, itemTemplate, 1, trigger.IsArgumentDefined("max"));
                ...
                //    character.WorldAccount.CountItensStaff += 1;
                //    character.WorldAccount.LastCountItenStaff = DateTime.Now;
                ...
                //    if (character != target)
                //    {
                //        character.SendInformationMessage(TextInformationTypeEnum.TEXT_INFORMATION_MESSAGE, 211, item.Template.Id, item.Guid, item.Stack, target.Name); //Agregaste %3 x {item,%1,%2} al inventario de %4.
                //        target.SendInformationMessage(TextInformationTypeEnum.TEXT_INFORMATION_MESSAGE, 114, item.Template.Id, item.Guid, item.Stack, character.Name); //El personal de Charly %4 agregó %3 x {item,%1,%2} a tu inventario.
                //    }
                //    else
                //    {
                //        character.SendInformationMessage(TextInformationTypeEnum.TEXT_INFORMATION_MESSAGE, 106, item.Template.Id, item.Guid, item.Stack); //Ha agregado %3 x {item,%1,%2} a su inventario.
                //    }
                //    #endregion
                //}
                else
                {
                    character.SendInformationMessage(TextInformationTypeEnum.TEXT_INFORMATION_MESSAGE, 242, character.Namedefault); //%1 você não tem permissão para realizar essa ação. Consulte seus direitos com o [DEV]Kenshin ou algum outro superior.
                }
            }
        }
                ...
                public override void Execute(TriggerBase trigger)
                {
                    ...
                    foreach (var item in items)
                    {
                        ...
                        if (cItem == null)
                            ...
                            trigger.Reply("Added '{0}'({1}) to '{2}' inventory.", item.Name, item.Id, target.Namedefault);
                    }
                }
            }
