...
namespace Stump.Server.WorldServer.Commands.Commands.GameMaster
{
    ...
    public class MonsterSpawnCommand : SubCommand
    {
        ...
        {
            var template = trigger.Get<List<MonsterTemplate>>("monster");
            ObjectPosition position = null;
            MonsterGroup group;
            foreach (var temp_template in template)
            {
                if (temp_template.Grades.Count <= trigger.Get<sbyte>("grade"))
                {
            ...
            //IEnumerable<MonsterGrade> grade = Enumerable.Empty<MonsterGrade>(); //= template[0].Grades[trigger.Get<sbyte>("grade")];
            List<MonsterGrade> grade = new List<MonsterGrade>();
            //template.ForEach(x => grade.Concat(new[] { x.Grades[trigger.Get<sbyte>("grade")] }));
            foreach (var temp_template in template)
            {
                // grade.Concat(new[] { temp_template.Grades[trigger.Get<sbyte>("grade")] });
                //grade.Concat(new[] { temp_template.Grades[1] });
                grade.Add(temp_template.Grades[trigger.Get<sbyte>("grade")]);
            }

            foreach (var temp_grade in grade)
            {
                if (temp_grade.Template.EntityLook == null)
                {
                    trigger.ReplyError("Cannot display this monster {0}", temp_grade.Template);
                    return;
                }
            }
            ...
            if (trigger.IsArgumentDefined("id"))
            {
                ...
                foreach (var temp_grade in grade)
                {
                    group.AddMonster(new Monster(temp_grade, group));
                }
            }
            else
                group = position.Map.SpawnMonsterGroup(grade, position);
            foreach (var temp_template in template)
                trigger.Reply("Monster '{0}' added to the group '{1}'", temp_template.Id, group.Id);
        }
    }
