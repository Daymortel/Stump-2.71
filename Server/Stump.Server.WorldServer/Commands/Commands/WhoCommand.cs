...
namespace Stump.Server.WorldServer.Commands.Commands
{
    ...
    public class StaffListCommand : CommandBase
    {
        ...
        {
            Aliases = new[] { "staff","stafflist" };
            RequiredRole = RoleEnum.Player;
            Description = "Retornar uma lista de staff conectado";
            Description_en = "Return a staff connected list";
            Description_es = "Devuelve una lista de personal conectado";
            Description_fr = "Renvoyer une liste de personnel connecté";
        }

        public override void Execute(TriggerBase trigger)
        {
            var list = World.Instance.GetCharacters(x => x.Account.UserGroupId > (int) RoleEnum.Moderator_Helper && x.Account.UserGroupId != (int)RoleEnum.Non_ADM && !x.Invisible);

            if (!list.Any())
            {
                if(((GameTrigger)trigger)!= null)
                switch (((GameTrigger)trigger).Character.Account.Lang)
                {
                    case "fr":
                            trigger.ReplyBold("Aucun résultat trouvé");
                            break;
                    case "es":
                            trigger.ReplyBold("No se han encontrado resultados");
                            break;
                    case "en":
                            trigger.ReplyBold("No results found");
                            break;
                    default:
                            trigger.ReplyBold("Nenhum resultado encontrado");
                            break;
                }
                else
                trigger.ReplyBold("Nenhum resultado encontrado");
