...
namespace Stump.Server.WorldServer.Commands
{
    public static class ParametersConverter
    {
        ...
        public static ConverterHandler<List<MonsterTemplate>> MonsterTemplateConverter = (entry, trigger) =>
        {
            int outvalue;
            if (int.TryParse(entry, out outvalue))
            {
                ...
                return new List<MonsterTemplate> { template };
            }
            List<int> listmonster = new List<int>();
            try
            {
                listmonster = !string.IsNullOrEmpty(entry) ? entry.FromCSV<int>(",").ToList() : null;
                if (listmonster != null)
                {
                    List<MonsterTemplate> listmonsterfin = new List<MonsterTemplate>();
                    foreach (var monster in listmonster)
                    {
                        var template = MonsterManager.Instance.GetTemplate(monster);

                        if (template == null)
                            throw new ConverterException(string.Format("'{0}' is not a valid monster template id", monster));
                        listmonsterfin.Add(template);
                    }
                    return listmonsterfin;
                }
            }
            catch { }
            ...
            return new List<MonsterTemplate> { templateByName };
        };
