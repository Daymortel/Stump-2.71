...
namespace Stump.Server.WorldServer.Database.Npcs.Replies
{
    ...
    public class TeleportReply : NpcReply
    {
        ...
        // private static readonly Logger loggerr = LogManager.GetCurrentClassLogger();
        ...
        public override bool Execute(Npc npc, Character character)
        {
            ...
            {
                if (GetPosition().Map.IsDungeon())
                {
                    character.Record.MapBeforeDungeonId = 0;
                    character.Record.CellBeforeDungeonId = 0;

                    if (!character.KnownDungeon.Contains(GetPosition().Map))
                        character.DiscoverDungeon(GetPosition().Map);
