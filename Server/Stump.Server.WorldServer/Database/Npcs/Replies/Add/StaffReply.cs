...
{
    [Discriminator("AddStaff", typeof(NpcReply), new Type[] { typeof(NpcReplyRecord) })]
    public class AddStaffReply : NpcReply
    {
        ...
        public AddStaffReply(NpcReplyRecord record) : base(record)
        {
        }
            ...
            else
            {
                ...
                else
                {
                    ...
                    else if (ReplyId == 34214)
                    {
                        ...
                        if (character.UserGroup.Role >=  RoleEnum.Moderator_Helper)
                        {
                            if (character.Account.Tokens <= 50000)
                            {
                                if (character.Inventory.CreateTokenItem(Amount, "Staff Ogrines Receiver"))
                                {
                                    character.SendInformationMessage(TextInformationTypeEnum.TEXT_INFORMATION_MESSAGE, 243, playerItem.Template.Id, playerItem.Guid, Amount);
                                    flag = true;
                                }
                                else
                                {
                            ...
                            else
                            {
                                ...
                                {
                                    case "fr":
                                        character.SendServerMessage("Vous avez déjà 50 000k Ogrines ou plus dans votre inventaire.");
                                        break;
                                    case "es":
                                        character.SendServerMessage("Ya tienes 50 000 000 ogrinas o más en tu inventario.");
                                        break;
                                    case "en":
                                        character.SendServerMessage("You already have 50,000k Ogrines or more in your inventory.");
                                        break;
                                    default:
                                        character.SendServerMessage("Você já possui 50.000k de Ogrines ou mais em seu inventário.");
                                        break;
                                }
                                #endregion
                                flag = false;
