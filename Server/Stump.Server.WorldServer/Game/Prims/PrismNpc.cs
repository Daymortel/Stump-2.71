    ...
    public class PrismNpc : RolePlayActor, IAutoMovedEntity, IContextDependant, IInteractNpc
    {
        ...
        public override bool StartMove(Path movementPath)
        {
            ...
            else
            {
                ...
                Map.ForEach(delegate (Character entry) { ContextHandler.SendGameMapMovementMessage(entry.Client, keys, this); });
                StopMove();
        ...
        {
            m_look = new ActorLook { BonesID = 2211 };
        ...
        {
            AlliancePrismInformation _alliancePrismInformation = new AlliancePrismInformation(
                state: (sbyte)PrismListenEnum.PRISM_LISTEN_ALL,
                placementDate: Date.GetUnixTimeStamp(),
                nuggetsCount: GetNuggetCount(),
                durability: 0,
                nextEvolutionDate: 0,
                alliance: Alliance.GetAllianceInformations()
                );

            return _alliancePrismInformation;
        }

        public PrismInformation GetAllianceInsiderPrismInformation()
        {
            AllianceInsiderPrismInformation _allianceInsider = new AllianceInsiderPrismInformation(
                state: (sbyte)PrismListenEnum.PRISM_LISTEN_ALL,
                placementDate: Date.GetUnixTimeStamp(),
                nuggetsCount: GetNuggetCount(),
                durability: 0,
        ...
        public PrismStateEnum GetStatePrism()
        {
            ...
            if (IsFighting && (Fighter.Fight.State == Fights.FightState.NotStarted || Fighter.Fight.State == Fights.FightState.Placement))
