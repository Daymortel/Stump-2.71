...
namespace Stump.Server.WorldServer.Game.Prisms
{
    public class PrismManager : DataManager<PrismManager>, ISaveable
    {
        ...
        public void Save()
        {
            foreach (var current in from prism in m_activePrisms select prism)
                current.Save();
        }
        ...
        {
            m_prismSpawns = Database.Query<WorldMapPrismRecord>(WorldMapPrismRelator.FetchQuery).ToDictionary(entry => entry.Id);
        ...
        public bool TryAddPrism(Character character, bool lazySave = true)
        {
            ...
            if (/* character.SubArea.Record.Capturable && */ !character.SubArea.HasPrism && character.Map.Prism == null && character.SubArea.Id != 10)//subarea 10 é a area do mapa de pvp, se ele ficar weakened todos ficam impedido de agredir :(
            {
                var position = character.Position.Clone();
                var npc = new PrismNpc(m_idProvider.Pop(), position.Map.GetNextContextualId(), position, character.Guild.Alliance, character);
