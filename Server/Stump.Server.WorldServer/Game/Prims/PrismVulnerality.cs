...
namespace Stump.Server.WorldServer.Game.Prisms
{
    public class PrismVulneranility : IJob
    {
        ...
        public KohUpdateMessage GetKoh(IEnumerable<Alliance> alliances, IEnumerable<short> allianceNbMembers, IEnumerable<int> allianceRoundWeigth, IEnumerable<sbyte> allianceMatchScore, IEnumerable<BasicAllianceInformations> winningMapList, uint allianceMapWinnerScore, uint allianceMapMyAllianceScore)
        {
            return new KohUpdateMessage();
        }

        public KohUpdateMessage GetMapByAllianceKoH(long mapId, AllianceKoH allianceChar, IEnumerable<Alliance> alliances,
        ...
        public KohUpdateMessage GetMapByAllianceKoH(long mapId, AllianceKoH allianceChar)
        {
        ...
        public void SendKoh()
        {
            ...
            var totalTime = _fightingAlliances.Values.Select(x => x.GetTotalTime());
            var fightAlliances = _fightingAlliances.Values;
            var mapAlliance = new Dictionary<int, Dictionary<long, KohUpdateMessage>>(fightAlliances.Count);

            foreach (var alliance in fightAlliances)
            {
                mapAlliance.Add(alliance.Alliance.Id, new Dictionary<long, KohUpdateMessage>());
