...
namespace Stump.Server.WorldServer.Game.Spells
{
    public class Spell
    {
        ...
        public ushort CurrentLevel
        {
            get
            {
                return ByLevel.ContainsKey(m_level) ? (ushort)m_level : (ushort)MaxLevel;
            }
            set
            {
                if (m_record != null)
                    m_record.Level = (short)value;
            ...
            try
            {
                ...
                if (m_record != null)
                {
                    ...
                    if (Template != null)
                    {
                        ...
                        {
                            var counter = 1;
                            var spellLevels = SpellManager.Instance.GetSpellLevels(Template);

                            if (spellLevels != null)
                            {
                                ByLevel = spellLevels.ToDictionary(entry => counter++);
                            }
                            else
                            {
                                logger.Error($"SpellLevels is null for SpellId: {m_id} SpellType: {SpellType.Id}");
                            }
                        }
                        else
                        {
                            logger.Error($"SpellType is null for SpellId: {m_id}");
                        }
                    }
                    else
                    {
                        logger.Error($"Template is null for SpellId: {m_id}");
                    }
                }
                else
                {
                    logger.Error("Spell record is null");
                }
            }
            catch (Exception ex)
            {
                logger.Error($"Spell: Exception occurred: {ex}");
            }
            #endregion
        }
            ...
            try
            {
                ...
                if (Template != null)
                {
                    ...
                    if (SpellType != null)
                    {
                        ...
                        {
                            logger.Error($"SpellLevels is null for SpellId: {id}");
                        }
                    }
                    else
                    {
                        logger.Error($"SpellType is null for SpellId: {id}");
                    }
                }
                else
                {
                    logger.Error($"Spell Template is null for SpellId: {id}");
                }
            }
            catch (Exception ex)
            {
            ...
            try
            {
                ...
                if (Template != null)
                {
                    ...
                    if (SpellType != null)
                    {
                        ...
                        {
                            logger.Error($"SpellLevels is null for SpellId: {m_id}");
                        }
                    }
                    else
                    {
                ...
                {
                    logger.Error("Spell Template is null");
                }
            }
            catch (Exception ex)
            {
