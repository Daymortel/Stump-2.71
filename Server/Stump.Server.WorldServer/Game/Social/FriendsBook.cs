...
namespace Stump.Server.WorldServer.Game.Social
{
    public class FriendsBook : IDisposable
    {
        ...
        public bool IsFriend(int targetid) => m_friends.Values.Any(x => x.Relation.TargetId == targetid);

        public bool IsIgnored(int targetid) => m_ignoreds.Values.Any(x => x.Relation.TargetId == targetid);
        ...
        public bool RemoveFriend(Friend friend)
        {
            ...
            if (m_friends.TryRemove(friend.Account.Id, out dummy))
            {
                ...
                FriendHandler.SendFriendDeleteResultMessage(Owner.Client, true, friend.Account.Nickname, friend.Account.Id);

                return true;
            }

            FriendHandler.SendFriendDeleteResultMessage(Owner.Client, false, friend.Account.Nickname, friend.Account.Id);
        ...
        public bool RemoveIgnored(Ignored ignored)
        {
            ...
            if (m_ignoreds.TryRemove(ignored.Account.Id, out dummy))
            {
                ...
                FriendHandler.SendIgnoredDeleteResultMessage(Owner.Client, true, ignored.Session, ignored.Account.Nickname, ignored.Account.Id);

                return true;
            }

            FriendHandler.SendIgnoredDeleteResultMessage(Owner.Client, false, ignored.Session, ignored.Account.Nickname, ignored.Account.Id);
        ...
        void OnCharacterLogIn(Character character)
        {
            ...
            if (m_friends.TryGetValue(character.Client.WorldAccount.Id, out friend))
            {
                ...
                if (WarnOnConnection)
                    BasicHandler.SendTextInformationMessage(Owner.Client, TextInformationTypeEnum.TEXT_INFORMATION_MESSAGE, 143, character.Client.WorldAccount.Nickname, character.Name); // %1 ({player,%2,%3}) est en ligne.
            }
        ...
        public void Load()
        {
            m_relations = new ConcurrentDictionary<int, AccountRelation>(WorldServer.Instance.DBAccessor.Database.Query<AccountRelation>
                (string.Format(AccountRelationRelator.FetchByAccount, Owner.Account.Id)).ToDictionary(x => x.TargetId, x => x).Where(x => x.Value != null));
            ...
            foreach (var relation in m_relations.Values)
            {
                ...
                var account = World.Instance.GetConnectedAccount(relation.TargetId) ?? AccountManager.Instance.FindById(relation.TargetId);
                ...
                if (account == null)
                {
                    ...
                    {
                        Console.WriteLine($"Error deleting relation: {ex.Message}");
                        // Aqui você pode escolher relançar a exceção, fazer log ou realizar outras ações de tratamento conforme necessário.
                    }
        ...
        public void Save()
        {
            try
            {
                ...
                foreach (var relation in m_relations.ToList())
                {
        ...
        public void CheckDC()
        {
            ...
            foreach (var ignored in Ignoreds.ToList())
            {
