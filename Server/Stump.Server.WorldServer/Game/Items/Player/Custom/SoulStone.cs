...
{
    [ItemType(ItemTypeEnum.PIERRE_D_ME_83)]
    public sealed class SoulStone : BasePlayerItem
    {
        private EffectDice m_soulStoneEffect;

        public SoulStone(Character owner, PlayerItemRecord record) : base(owner, record)
        {
        ...
        private void OnGeneratingResults(IFight obj)
        {
            ...
            if (fightPvM != null && fightPvM == obj && !fightPvM.IsPvMArenaFight && Owner.Fighter.HasWin() && Owner.Fighter.HasState((int)SpellStatesEnum.SOUL_SEEKER_2))
            {
