...
namespace Stump.Server.WorldServer.Game.Items.Player.Custom.LivingObjects
{
    public abstract class CommonLivingObject : BasePlayerItem
    {
        ...
        {
            if ((m_moodEffect = (EffectInteger)Effects.FirstOrDefault(x => x.EffectId == EffectsEnum.Effect_LivingObjectMood)) == null)
            {
                m_moodEffect = new EffectInteger(EffectsEnum.Effect_LivingObjectMood, 0);
                Effects.Add(m_moodEffect);
            }

            if ((m_selectedLevelEffect = (EffectInteger)Effects.FirstOrDefault(x => x.EffectId == EffectsEnum.Effect_LivingObjectSkin)) == null)
            {
                m_selectedLevelEffect = new EffectInteger(EffectsEnum.Effect_LivingObjectSkin, 1);
                Effects.Add(m_selectedLevelEffect);
            }

            if ((m_experienceEffect = (EffectInteger)Effects.FirstOrDefault(x => x.EffectId == EffectsEnum.Effect_LivingObjectLevel)) == null)
            {
                m_experienceEffect = new EffectInteger(EffectsEnum.Effect_LivingObjectLevel, 0);
                Effects.Add(m_experienceEffect);
            }

            if ((m_categoryEffect = (EffectInteger)Effects.FirstOrDefault(x => x.EffectId == EffectsEnum.Effect_LivingObjectCategory)) != null)
                return;
        ...
        {
            get { return m_experienceEffect.Value; }
            set
            {
                short level = 1;

                while (LevelsSteps.Length > level && LevelsSteps[level] <= value)
                    level++;

                if (m_record.Skins.Count <= 0 ? true : m_record.Skins.Count > level)
                {
                    m_experienceEffect.Value = value;
                }
                else if (level != LevelsSteps.Length)
                {
                    m_experienceEffect.Value = LevelsSteps[m_record.Skins.Count];
                    m_experienceEffect.Value -= 1;
                }
                else
                {
                    m_experienceEffect.Value = value;
        ...
        {
            get { return m_selectedLevelEffect.Value; }
            set
            {
