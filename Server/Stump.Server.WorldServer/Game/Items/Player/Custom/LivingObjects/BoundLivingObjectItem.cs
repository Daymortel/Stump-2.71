...
namespace Stump.Server.WorldServer.Game.Items.Player.Custom.LivingObjects
{
    ...
    public sealed class BoundLivingObjectItem : CommonLivingObject
    {
        ...
        public override bool Feed(BasePlayerItem food)
        {
            ...
            if (this.Level >= this.LivingObjectRecord.Skins.Count())
            {
                this.Owner.SendServerMessageLang(
                    "O seu item vivo já se encontra no level máximo.",
                    "Your living item is already at maximum level.",
                    "Tu objeto vivo ya está en el nivel máximo.",
                    "Votre objet vivant est déjà au niveau maximum.");

                return false;
            }

            var xp = (short)Math.Ceiling(food.Template.Level / 4d);
