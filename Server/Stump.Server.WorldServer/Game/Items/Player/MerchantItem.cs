...
namespace Stump.Server.WorldServer.Game.Items.Player
{
    public class MerchantItem : PersistantItem<PlayerMerchantItemRecord>
    {
        ...
        {
            return (compared.Template.Id == Template.Id && compared.Effects.CompareEnumerable(Effects));
        }
        ...
        {
            return new ObjectItemToSell((ushort)Template.Id, Effects.Select(x => x.GetObjectEffect()), (uint)Guid, (uint)Stack, (uint)Price);
        }
