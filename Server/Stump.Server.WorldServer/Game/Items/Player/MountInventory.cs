...
namespace Stump.Server.WorldServer.Game.Items.Player
{
    public class MountInventory : ItemsStorage<MountItem>
    {
        ...
        {
            Items = WorldServer.Instance.DBAccessor.Database.Query<MountItemRecord>(string.Format(MountItemRelator.FetchByOwner, Owner.EquippedMount.Id)).ToDictionary(x => x.Id, x => new MountItem(Owner, x));
        }
        ...
        public bool StoreKamas(long kamas)
        {
            if (kamas < 0)
                return false;
        ...
        public override void Save(ORM.Database database)
        {
            WorldServer.Instance.IOTaskPool.ExecuteInContext(() =>
            {
                ...
                {
                    logger.Error($"Error saving MountInventory: {ex.Message}");
                }
            });
        }
    }
