...
namespace Stump.Server.WorldServer.Game.Items.Player
{
    ...
    public sealed class Inventory : ItemsStorage<BasePlayerItem>, IDisposable
    {
        ...
        private uint[] items_type = { 16, 17, 10, 11, 1, 9, 82 };

        [Variable(true)]
        public const long MaxInventoryKamas = 9999999999999;
        ...
        readonly Dictionary<CharacterInventoryPositionEnum, List<BasePlayerItem>> m_itemsByPosition = new Dictionary<CharacterInventoryPositionEnum, List<BasePlayerItem>>
        {
            ...
            { CharacterInventoryPositionEnum.INVENTORY_POSITION_COMPANION, new List<BasePlayerItem>() }
        };


        readonly Dictionary<ItemSuperTypeEnum, CharacterInventoryPositionEnum[]> m_itemsPositioningRules = new Dictionary<ItemSuperTypeEnum, CharacterInventoryPositionEnum[]>
        {
            ...
            {ItemSuperTypeEnum.SUPERTYPE_COMPANION, new [] { CharacterInventoryPositionEnum.INVENTORY_POSITION_COMPANION } },
            {ItemSuperTypeEnum.SUPERTYPE_UNKNOWN_20, new [] { CharacterInventoryPositionEnum.INVENTORY_POSITION_FOLLOWER } },
        };
        ...
        public long WeightTotal => 1000 + (uint)(5 * Owner.Stats.Strength.Total) + (5 * ExperienceManager.Instance.GetCharacterLevel(Owner.Experience)) + (uint)Owner.Stats[PlayerFields.Weight].Total;
        ...
        {
            get { return Owner.Inventory.GetItems(CharacterInventoryPositionEnum.INVENTORY_POSITION_NOT_EQUIPED).FirstOrDefault(x => x.Template.Id == Inventory.TokenTemplateId); }
        ...
        internal void LoadInventory()
        {
            try
            {
                ...
                {
                    if (item.Template.Id == 30009 || item.Template.Id == 30010) //Item VIP Status
                        continue;

                    if (item is null)
                    {
                        logger.Error("LoadInventory - Item is null");
                        continue;
                    }
                ...
                foreach (var itemSet in GetEquipedItems().Where(entry => entry.Template.ItemSet != null).Select(entry => entry.Template.ItemSet).Distinct())
                {
                    if (itemSet is null)
                    {
                        logger.Error("LoadInventory - itemSet is null");
                        continue;
                    }
                ...
                if (TokenTemplate == null || !ActiveTokens || Owner.Client.Account.Tokens <= 0)
                    return;
            }
            catch (Exception ex)
            {
        ...
        internal void ReloadEffectsItemsEquiped()
        {
            try
            {
        ...
        public override long SetKamas(long amount)
        {
        ...
        public BasePlayerItem AddItem(ItemTemplate template, List<EffectBase> effects, int amount = 1)
        {
            ...
            if (itemStack != null && !itemStack.IsEquiped() && IsStackable(item, out itemStack))
            {
                ...
                if (item.Template.Id != Settings.TokenTemplateId)
                { Owner.SendInformationMessage(TextInformationTypeEnum.TEXT_INFORMATION_MESSAGE, 21, amount, itemStack.Template.Id); }
            ...
            else
            {
                ...
                if (item.Template.Id != Settings.TokenTemplateId)
                { Owner.SendInformationMessage(TextInformationTypeEnum.TEXT_INFORMATION_MESSAGE, 21, amount, template.Id); }
        ...
        public BasePlayerItem AddItem(ItemTemplate template, int amount = 1)
        {
            ...
            if (item != null && !item.IsEquiped() && IsStackable(item, out item))
            {
                ...
                if (item.Template.Id != Settings.TokenTemplateId)
                { Owner.SendInformationMessage(TextInformationTypeEnum.TEXT_INFORMATION_MESSAGE, 21, amount, item.Template.Id); }
        ...
        public override bool RemoveItem(BasePlayerItem item, bool delete = true, bool sendMessage = true)
        {
            ...
            try
            {
                ...
                result = item.OnRemoveItem() && base.RemoveItem(item, delete, sendMessage);
            }
            catch (Exception ex)
            {
        ...
        public bool CanTokenBlock()
        {
            bool Result;

            if (Settings.BlockTempOgrines == true || WorldServer.ServerInformation.Id == 2 && Owner.Account.UserGroupId <= 3)
                Result = true;
            else if (World.Instance.GetWorldStatus() == ServerStatusEnum.SAVING)
                Result = true;
            else if (!IPCAccessor.Instance.IsConnected)
                Result = true;
            else
                Result = false;
        ...
        //Create Tokens By: Kenshin
        public bool CreateTokenItem(int amount, string method)
        {
            BasePlayerItem tokens = Owner.Inventory.GetItems(CharacterInventoryPositionEnum.INVENTORY_POSITION_NOT_EQUIPED).FirstOrDefault(x => x.Template.Id == Inventory.TokenTemplateId);
            var item = ItemManager.Instance.CreatePlayerItem(Owner, TokenTemplateId, amount);
            int TokenAfter = Owner.Client.Account.Tokens;
            ...
            try
            {
                ...
                lock (Locker)
                {
                    if (tokens != null)
                    {
                        Owner.Inventory.StackItem(tokens, amount);
                        Owner.Inventory.NotifyItemStackChanged(tokens, amount);
                        Owner.Client.Account.Tokens = (int)tokens.Stack;
                        IPCAccessor.Instance.Send(new UpdateTokensMessage(Owner.Client.Account.Tokens, Owner.Client.Account.Id));
                    }
                    else
                    {
                        Owner.Inventory.AddItem(item);
                        Owner.Client.Account.Tokens = amount;
                        IPCAccessor.Instance.Send(new UpdateTokensMessage(Owner.Client.Account.Tokens, Owner.Client.Account.Id));
                    }
                }
                ...
                #region // ----------------- Sistema de Logs MongoDB Criação de Tokens by: Kenshin ---------------- //
                try
                {
                    var TokenName = "";
                    var CharacterRank = "Player";

                    if (Owner.Client.Account.UserGroupId >= 4 && Owner.Client.Account.UserGroupId <= 9)
                        CharacterRank = "Staff";

                    if (item.Template.Id == Settings.TokenTemplateId)
                        TokenName = "Ogrines";
                    else
                        TokenName = item.Template.Name;

                    var document = new BsonDocument
                        {
                          { "AccountUserGroup", CharacterRank },
                          { "AccountId", Owner.Account.Id },
                          { "AccountName", Owner.Account.Login },
                          ...
                          { "BeforeToken", Owner.Client.Account.Tokens },
                          { "HardwareId", Owner.Client.Account.LastHardwareId },
                ...
                {
                    Console.WriteLine("Erro no Mongologs das Create Tokens : " + e.Message);
                }
                #endregion
            ...
            {
                Console.WriteLine("Erro na Criação de Ogrines: " + e.Message);
                return false; // Retorna falso indicando falha
            }
        }

        //Remove Tokens By: Kenshin
        public bool RemoveTokenItem(int amount, string method)
        {
            int TokensAmount = Owner.Client.Account.Tokens;
            int TokenAfter = Owner.Client.Account.Tokens;
            ...
            try
            {
                ...
                lock (Locker)
                {
                    if (tokens != null)
                    {
                        Owner.Inventory.UnStackItem(tokens, amount);
                        Owner.Inventory.NotifyItemStackChanged(tokens, -amount);
                        TokensAmount = TokensAmount - amount;
                        Owner.Client.Account.Tokens = TokensAmount;
                        IPCAccessor.Instance.Send(new UpdateTokensMessage(TokensAmount, Owner.Client.Account.Id));
                    }
                    else
                    {
                        TokensAmount = 0;
                ...
                #region // ----------------- Sistema de Logs MongoDB Remove de Tokens by: Kenshin ---------------- //
                try
                {
                    var TokenName = "";
                    var CharacterRank = "Player";
                    ...
                    if (tokens.Template.Id == Settings.TokenTemplateId)
                        TokenName = "Ogrines";
                    else
                        TokenName = tokens.Template.Name;
                ...
                {
                    Console.WriteLine("Erro no Mongologs das Remove Tokens : " + e.Message);
                }
                #endregion
            ...
            {
                Console.WriteLine("Erro na Remoção de Ogrines: " + e.Message);
                return false; // Retorna falso indicando falha
            }
        }

        public void DeleteItemFromShortcuts(BasePlayerItem item)
        {
            var ShortcutItem = item.Owner.Shortcuts.ItemsShortcuts.FirstOrDefault(x => x.Value.ItemGuid == item.Guid);

            if (ShortcutItem.Value != null)
            {
                item.Owner.Shortcuts.RemoveShortcut(ShortcutBarEnum.GENERAL_SHORTCUT_BAR, ShortcutItem.Key);
            }
        }
        ...
        public bool CanEquip(BasePlayerItem item, CharacterInventoryPositionEnum position, bool send = true)
        {
            ...
            if (item.Template.Type.ItemType == ItemTypeEnum.BOUCLIER_82 && weapon != null && weapon.Template.TwoHanded)
            {
        ...
        public BasePlayerItem MoveItem(BasePlayerItem item, CharacterInventoryPositionEnum position, bool forceCanEquip = false)
        {
            ...
            {
                //return null;  YOU CAN USE RETURN NULL TO DEFINE TO DONT EQUIP NONE ITEN OR CAN DEFINE TO REMOVE INCARNATION ONLY
            ...
            var effects = item.Effects.Cast<EffectDice>();
            BasePlayerItem equipedItem;

            if (position != CharacterInventoryPositionEnum.INVENTORY_POSITION_NOT_EQUIPED && ((equipedItem = TryGetItem(position)) != null) && equipedItem.Template.TypeId != 300) // check if an item is already on the desired position
            {
            ...
            if (position == CharacterInventoryPositionEnum.INVENTORY_POSITION_NOT_EQUIPED && IsStackable(item, out stacktoitem) && stacktoitem != null) // check if we must stack the moved item
            {
            ...
            if (CeremonialRingsHandlers.TryGetFollowerId(item, out int followerRemoveId) && followerRemoveId > 0)
            {
                if (position == CharacterInventoryPositionEnum.INVENTORY_POSITION_NOT_EQUIPED)
                    CeremonialRingsHandlers.RemoveItemFollow(this.Owner, followerRemoveId, true);
                else if (position != CharacterInventoryPositionEnum.INVENTORY_POSITION_NOT_EQUIPED)
                    CeremonialRingsHandlers.SetItemFollow(this.Owner, followerRemoveId, true);
            }
        ...
        {
            if (itemToEquip.Template.Type.ItemType == ItemTypeEnum.DOFUS_23 || itemToEquip.Template.Type.ItemType == ItemTypeEnum.TROPHE_151)
            {
            ...
            if (itemToEquip.Template.Type.ItemType != ItemTypeEnum.ANNEAU_9)
                return;
        ...
        public void CheckICoinsPVM()
        {
            uint quantidade = 0;
            var item = ItemManager.Instance.TryGetTemplate(1328);
            BasePlayerItem[] test = GetItems(x => x.Template.Id == 1328).ToArray();
            if (test.Length > 1)
            {
                foreach (var coinpvm in test)
                {
                    // MoveItem(equipedItem, CharacterInventoryPositionEnum.INVENTORY_POSITION_NOT_EQUIPED);
                    quantidade += coinpvm.Stack;
                    RemoveItem(coinpvm, (int)coinpvm.Stack);
                }
                AddItem(item, (int)quantidade);
            }
            switch (Owner.Account.Lang)
            {
                case "fr":
                    Owner.SendServerMessage(string.Format("Vous avez actuellement {0} points de PVM", (int)quantidade));
                    break;
                case "es":
                    Owner.SendServerMessage(string.Format("Actualmente tienes {0} puntos de PVM", (int)quantidade));
                    break;
                case "en":
                    Owner.SendServerMessage(string.Format("You currently have {0} PVM points", (int)quantidade));
                    break;
                default:
                    Owner.SendServerMessage(string.Format("Você tem atualmente {0} pontos de PVM", (int)quantidade));
                    break;
            }
        }
        ...
        public void ApplyItemEffects(BasePlayerItem item, bool send = true, ItemEffectHandler.HandlerOperation? force = null, double? efficiency = null)
        {
            ...
            {
                logger.Error($"Error applying item effects: {ex.Message}");
            }
        }
        ...
        private void OnItemMoved(BasePlayerItem item, CharacterInventoryPositionEnum lastPosition)
        {
            ...
            if (lastPosition == CharacterInventoryPositionEnum.INVENTORY_POSITION_NOT_EQUIPED && CheckItemBug(item, false))
                return;
        ...
        public bool CheckItemBug(BasePlayerItem item, bool Move)
        {
            if (items_type.Contains(item.Template.TypeId) || item.Template.IsWeapon())
            {
                var StatusEffect = item.Effects.Where(x => !IsExotic(item, x));
                var ExoticEffect = item.Effects.Where(x => IsExotic(item, x));
                var ExoticDamages = ExoticEffect.Where(x => x.EffectId == EffectsEnum.Effect_MeeleDamageDonePercent || x.EffectId == EffectsEnum.Effect_RangedDamageDonePercent || x.EffectId == EffectsEnum.Effect_SpellDamageBonusPercent || x.EffectId == EffectsEnum.Effect_WeaponDamageDonePercent);

                #region Variaveis Exotic
                int ExoticEffectMeele = ExoticEffect.Any(x => x.EffectId == EffectsEnum.Effect_MeeleDamageDonePercent) ? (ExoticEffect.FirstOrDefault(x => x.EffectId == EffectsEnum.Effect_MeeleDamageDonePercent) as EffectInteger).Value : 0;
                int ExoticEffectRanged = ExoticEffect.Any(x => x.EffectId == EffectsEnum.Effect_RangedDamageDonePercent) ? (ExoticEffect.FirstOrDefault(x => x.EffectId == EffectsEnum.Effect_RangedDamageDonePercent) as EffectInteger).Value : 0;
                int ExoticEffectSpells = ExoticEffect.Any(x => x.EffectId == EffectsEnum.Effect_SpellDamageBonusPercent) ? (ExoticEffect.FirstOrDefault(x => x.EffectId == EffectsEnum.Effect_SpellDamageBonusPercent) as EffectInteger).Value : 0;
                int ExoticEffectWeapon = ExoticEffect.Any(x => x.EffectId == EffectsEnum.Effect_WeaponDamageDonePercent) ? (ExoticEffect.FirstOrDefault(x => x.EffectId == EffectsEnum.Effect_WeaponDamageDonePercent) as EffectInteger).Value : 0;
                int ExoticEffectRangedResis = ExoticEffect.Any(x => x.EffectId == EffectsEnum.Effect_RangedResistence) ? (ExoticEffect.FirstOrDefault(x => x.EffectId == EffectsEnum.Effect_RangedResistence) as EffectInteger).Value : 0;
                int ExoticEffectMeeleResis = ExoticEffect.Any(x => x.EffectId == EffectsEnum.Effect_MeeleResistence) ? (ExoticEffect.FirstOrDefault(x => x.EffectId == EffectsEnum.Effect_MeeleResistence) as EffectInteger).Value : 0;
                int ExoticEffectSpellsResis = ExoticEffect.Any(x => x.EffectId == EffectsEnum.Effect_SpellResistence) ? (ExoticEffect.FirstOrDefault(x => x.EffectId == EffectsEnum.Effect_SpellResistence) as EffectInteger).Value : 0;
                #endregion

                #region Variaveis Status
                int StatusEffectMeele = StatusEffect.Any(x => x.EffectId == EffectsEnum.Effect_MeeleDamageDonePercent) ? (StatusEffect.FirstOrDefault(x => x.EffectId == EffectsEnum.Effect_MeeleDamageDonePercent) as EffectInteger).Value : 0;
                int StatusEffectRanged = StatusEffect.Any(x => x.EffectId == EffectsEnum.Effect_RangedDamageDonePercent) ? (StatusEffect.FirstOrDefault(x => x.EffectId == EffectsEnum.Effect_RangedDamageDonePercent) as EffectInteger).Value : 0;
                int StatusEffectSpells = StatusEffect.Any(x => x.EffectId == EffectsEnum.Effect_SpellDamageBonusPercent) ? (StatusEffect.FirstOrDefault(x => x.EffectId == EffectsEnum.Effect_SpellDamageBonusPercent) as EffectInteger).Value : 0;
                int StatusEffectWeapon = StatusEffect.Any(x => x.EffectId == EffectsEnum.Effect_WeaponDamageDonePercent) ? (StatusEffect.FirstOrDefault(x => x.EffectId == EffectsEnum.Effect_WeaponDamageDonePercent) as EffectInteger).Value : 0;
                int StatusEffectRangedResis = StatusEffect.Any(x => x.EffectId == EffectsEnum.Effect_RangedResistence) ? (StatusEffect.FirstOrDefault(x => x.EffectId == EffectsEnum.Effect_RangedResistence) as EffectInteger).Value : 0;
                int StatusEffectMeeleResis = StatusEffect.Any(x => x.EffectId == EffectsEnum.Effect_MeeleResistence) ? (StatusEffect.FirstOrDefault(x => x.EffectId == EffectsEnum.Effect_MeeleResistence) as EffectInteger).Value : 0;
                int StatusEffectSpellsResis = StatusEffect.Any(x => x.EffectId == EffectsEnum.Effect_SpellResistence) ? (StatusEffect.FirstOrDefault(x => x.EffectId == EffectsEnum.Effect_SpellResistence) as EffectInteger).Value : 0;
                #endregion

                if (item.Effects.Exists(x => x.EffectId == EffectsEnum.Effect_NonExchangeable_981 && x.EffectId != EffectsEnum.Effect_CantFM) && ExoticEffectMeele > 1 || ExoticEffectRanged > 1 || ExoticEffectSpells > 1 || ExoticEffectWeapon > 1 || ExoticEffectRangedResis > 1 || ExoticEffectMeeleResis > 1 || ExoticEffectSpellsResis > 1)
                {
                    if (Move)
                    {
                ...
                else if (ExoticEffectMeele > 1 || ExoticEffectRanged > 1 || ExoticEffectSpells > 1 || ExoticEffectWeapon > 1 || ExoticEffectRangedResis > 1 || ExoticEffectMeeleResis > 1 || ExoticEffectSpellsResis > 1)
                {
                    if (Move)
                        Owner.Inventory.MoveItem(item, CharacterInventoryPositionEnum.INVENTORY_POSITION_NOT_EQUIPED);
                    ...
                    if (!item.Effects.Exists(x => x.EffectId == EffectsEnum.Effect_NonExchangeable_981))
                    {
                        item.Effects.Add(new EffectInteger(EffectsEnum.Effect_NonExchangeable_981, 0));
                        Owner.Inventory.RefreshItem(item);
                    }

                    #region // ----------------- Sistema de Logs MongoDB Captura de ItemBug by: Kenshin ---------------- //
                    try
                    {
                        var CharacterRank = "Player";
                        ...
                        var document = new BsonDocument
                                {
                                    ...
                                    { "CharacterName", Owner.Name },
                                    { "Status", "Exotic" },
                                    { "MeeleAmount", ExoticEffectMeele},
                                    { "RangedAmount", ExoticEffectRanged},
                                    { "SpellsAmount", ExoticEffectSpells},
                                    { "WeaponAmount", ExoticEffectWeapon},
                                    { "RangedResis", ExoticEffectRangedResis},
                                    { "MeeleResis", ExoticEffectMeeleResis},
                                    { "SpellsResis", ExoticEffectSpellsResis},
                                    { "ItemGuid", item.Guid},
                                    { "Date", DateTime.Now.ToString(CultureInfo.CreateSpecificCulture("pt-BR")) }
                                };

                        MongoLogger.Instance.Insert("Player_ItemBug", document);
                    }
                    catch (Exception e)
                    {
                        Console.WriteLine("Erro no Mongologs dos ItemsBugs : " + e.Message);
                    }
                    #endregion
                ...
                if (item.Effects.Exists(x => x.EffectId == EffectsEnum.Effect_NonExchangeable_981 && x.EffectId != EffectsEnum.Effect_CantFM) && StatusEffectMeele > 16 || StatusEffectRanged > 16 || StatusEffectSpells > 16 || StatusEffectWeapon > 16 || StatusEffectRangedResis > 16 || StatusEffectMeeleResis > 16 || StatusEffectSpellsResis > 16)
                {
                    if (Move)
                    {
                        Owner.Inventory.MoveItem(item, CharacterInventoryPositionEnum.INVENTORY_POSITION_NOT_EQUIPED);
                ...
                else if (StatusEffectMeele > 16 || StatusEffectRanged > 16 || StatusEffectSpells > 16 || StatusEffectWeapon > 16 || StatusEffectRangedResis > 16 || StatusEffectMeeleResis > 16 || StatusEffectSpellsResis > 16)
                {
                    if (Move)
                        Owner.Inventory.MoveItem(item, CharacterInventoryPositionEnum.INVENTORY_POSITION_NOT_EQUIPED);
                        ...
                        var document = new BsonDocument
                                {
                                    ...
                                    { "CharacterName", Owner.Name },
                                    { "Status", "Status" },
                                    { "MeeleAmount", StatusEffectMeele},
                                    { "RangedAmount", StatusEffectRanged},
                                    { "SpellsAmount", StatusEffectSpells},
                                    { "WeaponAmount", StatusEffectWeapon},
                                    { "RangedResis", StatusEffectRangedResis},
                                    { "MeeleResis", StatusEffectMeeleResis},
                                    { "SpellsResis", StatusEffectSpellsResis},
                                    { "ItemGuid", item.Guid},
                                    { "Date", DateTime.Now.ToString(CultureInfo.CreateSpecificCulture("pt-BR")) }
                                };
        ...
        public bool IsExotic(BasePlayerItem item, EffectBase effect)
        {
            return item.Template.Effects.All(x => x.EffectId != effect.EffectId);
        }
        ...
        protected override void OnKamasAmountChanged(long amount)
        {
        ...
        public BasePlayerItem[] GetEquipedItem(uint ObjtUi)
        {
            return (from entry in Items
                    where entry.Value.IsEquiped() && entry.Value.Guid == ObjtUi
                    select entry.Value).ToArray();
        }
        ...
        public Tuple<short?, bool> GetPetSkin()
        {
            ...
            return Tuple.Create((short?)pet.AppearanceId, pet.Template.TypeId == (int)ItemTypeEnum.FAMILIER_18);
        }
            ...
            WorldServer.Instance.IOTaskPool.ExecuteInContext(() =>
            {
                ...
                lock (Locker)
                {
                    try
                    {
                        ...
                        using (var enumerator = (from item in Items where Tokens == null || item.Value != Tokens select item).GetEnumerator())
                        {
                    ...
                    {
                        logger.Error($"Error saving Inventory 1: {ex.Message}");
                    }
                }
            });
        }
        ...
        public void Save(ORM.Database database, bool updateAccount)
        {
            lock (Locker)
            {
                try
                {
                    foreach (var item in Items.Where(item => Tokens == null || item.Value != Tokens).Where(item => !item.Value.IsTemporarily).ToList())
                    {
                        if (item.Value == null)
                            continue;
                    ...
                    while (ItemsToDelete.ToList().Count > 0)
                    {
                    ...
                    while (PresetsToDelete.ToList().Count > 0)
                    {
                    ...
                    Owner.Client.Account.Tokens = Tokens == null ? 0 : (int)Tokens.Stack;

                    if (updateAccount)
                    {
                        if (Owner.WorldAccount != null)
                            database.Update(Owner.WorldAccount);
                    }
                }
                catch (Exception ex)
                {
                    logger.Error($"Error saving Inventory 2: {ex.Message}");
                }
            }
        }
        #endregion
