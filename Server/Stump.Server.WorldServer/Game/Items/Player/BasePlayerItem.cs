...
namespace Stump.Server.WorldServer.Game.Items.Player
{
    public abstract class BasePlayerItem : PersistantItem<PlayerItemRecord>
    {
        ...
        public List<ItemSuperTypeEnum> NotValidToExchange = new List<ItemSuperTypeEnum>() {
            ItemSuperTypeEnum.SUPERTYPE_UNKNOWN_0,
            ItemSuperTypeEnum.SUPERTYPE_QUEST,
            ItemSuperTypeEnum.SUPERTYPE_UNKNOWN_17,
            ItemSuperTypeEnum.SUPERTYPE_UNKNOWN_18,
            ItemSuperTypeEnum.SUPERTYPE_UNKNOWN_19,
            ItemSuperTypeEnum.SUPERTYPE_UNKNOWN_20,
        };
        ...
        #region Constructors
        protected BasePlayerItem(Character owner, PlayerItemRecord record) : base(record)
        {
            try
            {
                ...
                if (IsEquiped() && Template.FavoriteSubAreas.Length > 0)// Length
                    Owner.EnterMap += OnEnterMap;

                if (Effects.Exists(x => x.EffectId == EffectsEnum.Effect_Exchangeable && x is EffectInteger))
                {
                    int index = Effects.FindIndex(x => x.EffectId == EffectsEnum.Effect_Exchangeable);

                    if (index >= 0)
                        Effects[index] = new EffectDate(EffectsEnum.Effect_Exchangeable, DateTime.Now);
                }

                // FORCE ITENS THAT NO HAD THIS EFFECT AND ARE INCARNATION
                if (Template.IsWeapon() && !Effects.Exists(x => x.EffectId == EffectsEnum.Effect_669 && x is EffectInteger))
                {
                    var record_inca = IncarnationManager.Instance.GetCustomIncarnationRecordByItem(Template.Id);

                    if (record_inca != null)
                    {
                        Effects.Add(new EffectInteger(EffectsEnum.Effect_669, (short)record_inca.Id));
        ...
        {
            if (NotValidToExchange.Contains(Template.Type.SuperType))
                return true;

            if (IsTokenItem())
                return true;

            if (Effects.Exists(x => x is EffectDate && x.EffectId == EffectsEnum.Effect_Exchangeable && (x as EffectDate).GetDate() > DateTime.Now))
                return true;

            return false;//Effects.Any(x => x.EffectId == EffectsEnum.Effect_NonExchangeable_982);
        }

        public virtual bool IsAppearanceItemEquiped()
        {
            if (Template.Effects.Exists(x => x.EffectId == EffectsEnum.Effect_Appearance || x.EffectId == EffectsEnum.Effect_Apparence_Wrapper || x.EffectId == EffectsEnum.Effect_LivingObjectId))
                return true;
        ...
        {
            if (Effects.Exists(x => x.EffectId == EffectsEnum.Effect_NonExchangeable_981))
                return true;
            ...
            return false;//Effects.Any(x => x.EffectId == EffectsEnum.Effect_NonExchangeable_981);
        }
        ...
        {
            if (!unequip && Template.FavoriteSubAreas.Length > 0)// Length
            {
                Owner.EnterMap += OnEnterMap;
            }
            else if (unequip && Template.FavoriteSubAreas.Length > 0)// Length
            {
                Owner.EnterMap -= OnEnterMap;
            }
            return true;
        ...
        internal void SetEffects(List<EffectBase> effects)
        {
            Record.Effects = effects;
        }

        public List<EffectBase> GetExoEffects()
        {
            return Effects.Where(x => !Template.Effects.Contains(x)).ToList();
        }
