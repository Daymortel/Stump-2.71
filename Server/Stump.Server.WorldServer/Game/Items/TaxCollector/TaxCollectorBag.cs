...
namespace Stump.Server.WorldServer.Game.Items.TaxCollector
{
    public class TaxCollectorBag : PersistantItemsCollection<TaxCollectorItem>
    {
        ...
        public void LoadRecord()
        {
            ...
            var records = ItemManager.Instance.FindTaxCollectorItems(Owner.GlobalId);
        ...
        public void DeleteBag()
        {
            try
            {
                ...
                Console.WriteLine("Deletion successful. Bag has been successfully deleted.");
            }
            catch (Exception ex)
            {
                Console.WriteLine($"Error during bag deletion: {ex.Message}. The bag could not be deleted.");
            }
        }
