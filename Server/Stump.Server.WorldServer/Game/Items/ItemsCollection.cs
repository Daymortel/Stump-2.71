...
namespace Stump.Server.WorldServer.Game.Items
{
    public class ItemsCollection<T> : IEnumerable<T> where T : IItem
    {
        ...
        public virtual int CountSpecificType(int type) => Items.Values.Count(x => x.Template.TypeId == type);
    ...
    public class PersistantItemsCollection<T> : ItemsCollection<T> where T : IPersistantItem
    {
        ...
        public virtual void Save(ORM.Database database)
        {
            lock (Locker)
            {
                try
                {
                    foreach (var item in Items.Where(item => !item.Value.IsTemporarily).ToList())
                    {
                        if (item.Value == null)
                            continue;
                ...
                {
                    logger.Error($"Error saving ItemsCollection: {ex.Message}");
                }
            }
        }
        #endregion
