...
namespace Stump.Server.WorldServer.Game.Items.BidHouse
{
    public class BidHouseManager : DataManager<BidHouseManager>, ISaveable
    {
        ...
        public List<BidHouseItem> GetBidHouseItems(ItemTypeEnum type, int maxItemLevel) => m_bidHouseItems.Where(x => x.Template.TypeId == (int)type && x.Template.Level <= maxItemLevel && !x.Sold).GroupBy(x => x.Template.Id).Select(x => x.First()).ToList();
        ...
        public List<BidHouseItem> GetBidHouseItems(int ownerId, IEnumerable<int> types) => m_bidHouseItems.Where(x => x.Record.OwnerId == ownerId && types.Contains((int)x.Template.TypeId) && !x.Sold).ToList();
