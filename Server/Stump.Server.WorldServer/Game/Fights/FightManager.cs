...
namespace Stump.Server.WorldServer.Game.Fights
{
    public class FightManager : EntityManager<FightManager, IFight>
    {
        ...
        public BreachFight CreateBreachFight(Map map, Character character)
        {
            var challengersTeam = new FightPlayerTeam(TeamEnum.TEAM_CHALLENGER, map.GetRedFightPlacement());
            var defendersTeam = new FightMonsterTeam(TeamEnum.TEAM_DEFENDER, map.GetBlueFightPlacement());
        ...
        public FightAgression CreateAgressionFightKoh(Map map)
        {
            var redTeam = new FightPlayerTeam(TeamEnum.TEAM_CHALLENGER, map.GetRedFightPlacement());
            var blueTeam = new FightPlayerTeam(TeamEnum.TEAM_DEFENDER, map.GetBlueFightPlacement());
            var fightAgression = new FightAgressionKoH(m_idProvider.Pop(), map, blueTeam, redTeam);
            AddEntity(fightAgression.Id, fightAgression);
        ...
        public FightPvMr CreatePvMrFight(Map map)
        {
            var redTeam = new FightPrismAttackersTeam(0, map.GetRedFightPlacement());
            var blueTeam = new FightPrismDefenderTeam((TeamEnum)1, map.GetBlueFightPlacement());
            var fightPvT = new FightPvMr(m_idProvider.Pop(), map, blueTeam, redTeam);
            AddEntity(fightPvT.Id, fightPvT);
            return fightPvT;
        }
    }
}