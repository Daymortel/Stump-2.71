...
namespace Stump.Server.WorldServer.Game.Fights.History
{
    public class SpellHistory
    {
        ...
        public bool CanCastSpell(SpellLevelTemplate spell, Cell targetedCell)
        {
            ...
            if (Spell != null)
            {
                AdditionalCastPerTarget = Spell.AdditionalCastPerTarget > 0 ? Spell.AdditionalCastPerTarget : 0;
            }
        ...
        {
            if (spell.GlobalCooldown != 0 && Owner.Team.Fighters.OfType<CharacterFighter>().Any(x => x.SpellHistory.GetMostRecentEntry(spell) != null && x.SpellHistory.GetMostRecentEntry(spell).IsGlobalCooldownActive(CurrentRound)))
                return false;
