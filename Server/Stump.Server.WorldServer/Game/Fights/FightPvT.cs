...
namespace Stump.Server.WorldServer.Game.Fights
{
    ...
    public class FightPvT : Fight<FightTaxCollectorDefenderTeam, FightTaxCollectorAttackersTeam>
    {
        ...
        public object RedTeam { get; internal set; }
        public object BlueTeam { get; internal set; }
        ...
        public override void StartPlacement()
        {
            ...
            if (TaxCollector.TaxCollectorNpc.Guild.Alliance != null)
            {
                TaxCollectorHandler.SendTaxCollectorAttackedMessage(TaxCollector.TaxCollectorNpc.Guild.Alliance.Clients, TaxCollector.TaxCollectorNpc);

                foreach (var a in TaxCollector.TaxCollectorNpc.Guild.Alliance.m_guilds)
                {
                    TaxCollectorHandler.SendTaxCollectorListMessage(TaxCollector.TaxCollectorNpc.Guild.Alliance.Clients, a.Value);
                }
            }
            else
            {
                TaxCollectorHandler.SendTaxCollectorAttackedMessage(TaxCollector.TaxCollectorNpc.Guild.Clients, TaxCollector.TaxCollectorNpc);
                TaxCollectorHandler.SendTaxCollectorListMessage(TaxCollector.TaxCollectorNpc.Guild.Clients, TaxCollector.TaxCollectorNpc.Guild);
            }
        }
            ...
            foreach (var defender in DefendersQueue)
            {
                ...
                defender.Area.ExecuteInContext(() =>
                {
                    ...
                    Map.Area.ExecuteInContext(() =>
                    {
                        ...
                        if (DefendersQueue.All(x => DefendersTeam.Fighters.OfType<CharacterFighter>().Any(y => y.Character == x)))
        ...
        public FighterRefusedReasonEnum AddDefender(Character character)
        {
            ...
            if (TaxCollector.TaxCollectorNpc.Guild.Alliance != null)
            {
                foreach (var a in TaxCollector.TaxCollectorNpc.Guild.Alliance.m_guilds.Values)
                {
                    if (a.Id == character.Guild.Id)
                    {
                        alliance = true;
                    }
                }
            ...
            {
                if (character.Guild.Id != TaxCollector.TaxCollectorNpc.Guild.Id)
                    return FighterRefusedReasonEnum.WRONG_GUILD;
            }

           if(alliance == false && TaxCollector.TaxCollectorNpc.Guild.Alliance != null)
                return FighterRefusedReasonEnum.WRONG_ALLIANCE;
            ...
            if (ChallengersTeam.Fighters.OfType<CharacterFighter>().Any(x => x.Character.Client.IP == character.Client.IP))
                return FighterRefusedReasonEnum.MULTIACCOUNT_NOT_ALLOWED;

            if (ChallengersTeam.Fighters.OfType<CharacterFighter>().Contains(character.Fighter))
                return FighterRefusedReasonEnum.MULTIACCOUNT_NOT_ALLOWED;
            ...
            if(character.Guild.Alliance != null)
                TaxCollectorHandler.SendGuildFightPlayersHelpersJoinMessage(character.Guild.Alliance.Clients , TaxCollector.TaxCollectorNpc, character);
            else
        ...
        public bool RemoveDefender(Character character)
        {
            ...
            if (character.Guild.Alliance != null)
                TaxCollectorHandler.SendGuildFightPlayersHelpersLeaveMessage(character.Guild.Alliance.Clients, TaxCollector.TaxCollectorNpc, character);
            else
        ...
        public override bool CanChangePosition(FightActor fighter, Cell cell) => base.CanChangePosition(fighter, cell) && ((IsAttackersPlacementPhase && fighter.Team == ChallengersTeam) || (IsDefendersPlacementPhase && fighter.Team == DefendersTeam));
        ...
        protected override void OnFighterAdded(FightTeam team, FightActor actor)
        {
            ...
            {
                TaxCollectorHandler.SendGuildFightPlayersEnemiesListMessage(TaxCollector.TaxCollectorNpc.Guild.Clients, TaxCollector.TaxCollectorNpc, ChallengersTeam.Fighters.OfType<CharacterFighter>().Select(x => x.Character));
            }
        ...
        protected override void OnFighterRemoved(FightTeam team, FightActor actor)
        {
            ...
            if (State == FightState.Placement && team == ChallengersTeam && characterFighter != null)
            {
                TaxCollectorHandler.SendGuildFightPlayersEnemyRemoveMessage(TaxCollector.TaxCollectorNpc.Guild.Clients, TaxCollector.TaxCollectorNpc, characterFighter.Character);
            }

            TaxCollectorFighter taxCollector = actor as TaxCollectorFighter;

            if (taxCollector != null && taxCollector.IsAlive())
        ...
        {
            TaxCollectorHandler.SendTaxCollectorAttackedResultMessage(TaxCollector.TaxCollectorNpc.Guild.Clients, Winners != DefendersTeam && !draw, TaxCollector.TaxCollectorNpc);
        ...
        protected override List<IFightResult> GetResults()
        {
            ...
            var looters = ChallengersTeam.GetAllFightersWithLeavers().Where(entry => entry.HasResult).Select(entry => entry.GetFightResult()).OrderByDescending(entry => entry.Prospecting);
            ...
            {
                var charactersdefenders = Winners.Fighters.OfType<CharacterFighter>().Where(x => x.Character.GuildMember != null);

                List<Guild> GuildsDefenders = new List<Guild>();

                foreach (CharacterFighter owner in charactersdefenders)
                {
                    if (owner.Character.GuildMember != null && !GuildsDefenders.Contains(owner.Character.Guild))
                    {
                        owner.Character.Guild.UpdateDefenders();
                        GuildsDefenders.Add(owner.Character.Guild);
                    }
                }
            }
            ...
            var characters = Winners.Fighters.OfType<CharacterFighter>().Where(x => x.Character.GuildMember != null);
            ...
            foreach (CharacterFighter owner in characters)
            {
                if (owner.Character.GuildMember != null && !Guilds.Contains(owner.Character.Guild))
                {
                    owner.Character.Guild.UpdateKills();
                    Guilds.Add(owner.Character.Guild);
                }
            }
        ...
        protected override void SendGameFightJoinMessage(CharacterFighter fighter)
        {
            ...
            ContextHandler.SendGameFightJoinMessage(fighter.Character.Client, CanCancelFight(), (fighter.Team == ChallengersTeam && IsAttackersPlacementPhase) || (fighter.Team == DefendersTeam && IsDefendersPlacementPhase), IsStarted, timer, FightType);
        }
        ...
        public TimeSpan GetPlacementTimeLeft(FightActor fighter)
        {
            ...
            if ((fighter.Team == ChallengersTeam && IsAttackersPlacementPhase) || (fighter.Team == DefendersTeam && IsDefendersPlacementPhase))
                return m_placementTimer.NextTick - DateTime.Now;
