...
namespace Stump.Server.WorldServer.Game.Fights.Triggers
{
    public class Portal : MarkTrigger
    {
        ...
        public Portal(short id, FightActor caster, Spell castedSpell, Cell centerCell, EffectDice originEffect, SpellShapeEnum shapes, byte minsize, byte size, Color color, Spell portalSpell) : base(id, caster, castedSpell, originEffect, centerCell, new MarkShape(caster.Fight, centerCell, shapes, GameActionMarkCellsTypeEnum.CELLS_CIRCLE, minsize, size, color))
        {
        ...
        {
            return new GameActionMark(Caster.Id, (sbyte)Caster.Team.Id, CastedSpell.Id, (sbyte)CastedSpell.CurrentLevel, Id, (sbyte)Type, CenterCell.Id, Shape.GetGameActionMarkedCells(), IsActive);
        }

        public override GameActionMark GetHiddenGameActionMark() => new GameActionMark(Caster.Id, (sbyte)Caster.Team.Id, CastedSpell.Id, (sbyte)CastedSpell.CurrentLevel, Id, (sbyte)Type, CenterCell.Id, new GameActionMarkedCell[0], IsActive);
            ...
            if (portal != null && portal.IsInGameActive)
            {
                if (portal.IsActive && Fight.PortalsManager.GetActivableGamePortalsCount(portal.Caster.Team) >= 1)
                {
                    ...
                    if (links.Count > 0)
                    {
                        ...
                        var spellCastHandler = Singleton<SpellManager>.Instance.GetSpellCastHandler(Fight.FighterPlaying, PortalSpell, actor.Cell, false);
