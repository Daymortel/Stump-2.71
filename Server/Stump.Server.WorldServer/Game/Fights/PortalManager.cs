...
namespace Stump.Server.WorldServer.Game.Fights
{
    public class FightPortalsManager
    {
        ...
        {
            if (Fight.GetTriggers().OfType<Portal>() == null || Fight.GetTriggers().OfType<Portal>().Count() < 1)
                return;
        ...
        public void RefreshClientsPortals()
        {
            try
            {
                if (Fight.GetTriggers().OfType<Portal>() == null)
                    return;
        ...
        public SpellCastInformations TeleportCast(SpellCastInformations cast, FightActor caster)
        {
            ...
            if (portal != null)
            {
                if (GetActivableGamePortalsCount(portal.Caster.Team) < 2)
                    return cast;
                ...
                if (portal.IsActive)
                {
                    ...
                    if (links.Count > 0)
                    {
                        try
                        {
                            var finalPortal = links?.LastOrDefault();

                            if (finalPortal != null)
                            {
                                var finalPortalCell = Map.GetCell((int)finalPortal);
                                ...
                                if (point2 != null)
                                {
                                    ...
                                    cast.CastCell = Map.Cells[(int)finalPortal];
                        ...
                        {
                            Console.WriteLine("Ocorreu um erro ao tentar executar a ação. Detalhes: " + ex.Message);
                        }
                    }
                }
            }
