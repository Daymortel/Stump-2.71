...
namespace Stump.Server.WorldServer.Game.Fights.Buffs
{
    ...
    public class TriggerBuff : Buff
    {
        public static readonly Dictionary<string, BuffTriggerType> m_triggersMapping = new Dictionary<string, BuffTriggerType>()
        {
            ...
            //Não existe mais Spells que usem essas strings
        ...
        public TriggerBuff(int id, FightActor target, FightActor caster, SpellEffectHandler effect, Spell spell, Spell parentSpell, bool critical, FightDispellableEnum dispelable, int priority,
            TriggerBuffApplyHandler applyTrigger, TriggerBuffRemoveHandler removeTrigger = null) : base(id, target, caster, effect, spell, critical, dispelable)
        {
        ...
        public bool ShouldTrigger(BuffTriggerType type, object token = null) => Delay == 0 && Triggers.Any(x => x.Type == type && (x.Parameter == null || x.Parameter.Equals(token)));
