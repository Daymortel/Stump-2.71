...
namespace Stump.Server.WorldServer.Game.Jobs
{
    public class Job
    {
        ...
        private int GetXpPerLevel(RecipeRecord recipe, int level)
        {
            ...
            double RatePercent = Owner.Client.UserGroup.Role >= RoleEnum.Gold_Vip ? Rates.GoldJobXpRate : Owner.Client.UserGroup.Role == RoleEnum.Vip ? Rates.VipJobXpRate : Rates.JobXpRate;
            ...
            xpPerLevel *= Math.Truncate(RatePercent / 100);
        ...
        public JobExperience GetJobExperience() => new JobExperience((sbyte)Template.Id, (byte)Level, (ulong)Experience, (ulong)LowerBoundExperience, (ulong)UpperBoundExperience);

        public JobDescription GetJobDescription() => new JobDescription((sbyte)Template.Id, Template.Skills.Where(x => x.LevelMin <= Level).Select(x => GetSkillActionDescription(x)));

        public JobCrafterDirectorySettings GetJobCrafterDirectorySettings() => new JobCrafterDirectorySettings((sbyte)Template.Id, (byte)MinLevelCraftSetting, WorkForFree);
        
        //Version 2.61 Corrigir
        public JobCrafterDirectoryListEntry GetJobCrafterDirectoryListEntry()
        {
            return new JobCrafterDirectoryListEntry(new JobCrafterDirectoryEntryPlayerInfo(
                         (ulong)Owner.Id,
                         Owner.Name,
                         (sbyte)Owner.AlignmentSide,
                         (sbyte)Owner.Breed.Id,
                         Owner.Sex != SexTypeEnum.SEX_FEMALE,
                         Owner.Map.AvailableJobs.Contains(Template),
                         (short)Owner.Map.Position.X,
                         (short)Owner.Map.Position.Y,
                         Owner.Map.Id,
                         (ushort)Owner.SubArea.Id,
                         Owner.PrestigeRank == 10, //TODO
                         Owner.Status),
        ...
        public void Save(ORM.Database database)
        {
            WorldServer.Instance.IOTaskPool.AddMessage(() =>
            {
                ...
                {
                    logger.Error($"Error saving Job: {ex.Message}");
                }
            });
        }
        #endregion
