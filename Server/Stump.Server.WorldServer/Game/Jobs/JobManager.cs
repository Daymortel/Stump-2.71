...
namespace Stump.Server.WorldServer.Game.Jobs
{
    public class JobManager : DataManager<JobManager>, ISaveable
    {
        ...
        {
            if (itemId == 0 || ownerId == 0 || amount == 0)
                return;
        ...
        public void Save()
        {
            try
            {
                ...
                foreach (var id in dbIds.Distinct().Where(i => i.IsUpdated))
                {
                    if (id.ItemId == 0 || id.OwnerId == 0 || id.Amount == 0)
                        continue;
                    ...
                    if (record != null)
                    {
                        ...
                        {
                            Console.WriteLine("Erro ao salvar registro atualizado: " + ex.Message);
                        }
                    }
                }

                foreach (var id in dbIds.Distinct().Where(i => i.IsNew))
                {
                    if (id.ItemId == 0 || id.OwnerId == 0 || id.Amount == 0)
                        continue;
                    ...
                    try
                    {
                        CraftItemRecord record = new CraftItemRecord()
                        {
                            ItemId = id.ItemId,
                            Amount = id.Amount,
                            OwnerId = id.OwnerId,
                    ...
                    {
                        Console.WriteLine("Erro ao inserir novo registro: " + ex.Message);
                    }
                }
            ...
            {
                Console.WriteLine("Erro ao executar a transação do banco de dados: " + ex.Message);
            }
        }
        #endregion
    }
