...
namespace Stump.Server.WorldServer.Game.Quests.Objectives
{
    public class CraftItemObjective : QuestObjective
    {
        ...
        public CraftItemObjective(Character character, QuestObjectiveTemplate template, bool finished, int itemId, int amount) : base(character, template, finished)
        {
            Item = ItemManager.Instance.TryGetTemplate(itemId);
            Amount = amount;
        }

        public CraftItemObjective(Character character, QuestObjectiveTemplate template, QuestObjectiveStatus finished, int itemId, int amount) : base(character, template, finished)
        {
            Item = ItemManager.Instance.TryGetTemplate(itemId);
            Amount = amount;
        }
