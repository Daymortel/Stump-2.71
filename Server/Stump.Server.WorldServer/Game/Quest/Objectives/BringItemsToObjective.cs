...
namespace Stump.Server.WorldServer.Game.Quests.Objectives
{
    ...
    {
        public BringItemsToObjective(Character character, QuestObjectiveTemplate template, bool finished, int npcId, int amount, int itemId, bool delete) : base(character, template, finished)
        {
        ...
        public BringItemsToObjective(Character character, QuestObjectiveTemplate template, QuestObjectiveStatus status, int npcId, int amount, int itemId, bool delete) : base(character, template, status)
        {
            Npc = NpcManager.Instance.GetNpcTemplate(npcId);
            Amount = amount;
            Item = ItemManager.Instance.TryGetTemplate(itemId);
