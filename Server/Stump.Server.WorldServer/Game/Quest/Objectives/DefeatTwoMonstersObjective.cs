...
{
    public class DefeatTwoMonstersObjective : QuestObjective
    {
        private int m_completion;

        public DefeatTwoMonstersObjective(Character character, QuestObjectiveTemplate template, bool finished, int monsterId, int amount) : base(character, template, finished)
        {
            Monster = MonsterManager.Instance.GetTemplate(monsterId);
            Amount = amount;
        }

        public DefeatTwoMonstersObjective(Character character, QuestObjectiveTemplate template, QuestObjectiveStatus finished, int monsterId, int amount) : base(character, template, finished)
        {
            Monster = MonsterManager.Instance.GetTemplate(monsterId);
            Amount = amount;
        }
        ...
        {
            if (!(fighter.Fight is FightPvM || fighter.Fight is FightPvD))
                return;

            if (!(fighter.Fight is FightPvD) && (fighter.Fight as FightPvM).IsPvMArenaFight)
                return;
