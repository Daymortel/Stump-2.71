...
namespace Stump.Server.WorldServer.Game.Quests.Objectives
{
    public class DefeatMonstersObjective : QuestObjective
    {
        ...
        public DefeatMonstersObjective(Character character, QuestObjectiveTemplate template, bool finished, int monsterId, int amount) : base(character, template, finished)
        {
            Monster = MonsterManager.Instance.GetTemplate(monsterId);
            Amount = amount;
        }

        public DefeatMonstersObjective(Character character, QuestObjectiveTemplate template, QuestObjectiveStatus finished, int monsterId, int amount) : base(character, template, finished)
        {
            Monster = MonsterManager.Instance.GetTemplate(monsterId);
            Amount = amount;
        }
