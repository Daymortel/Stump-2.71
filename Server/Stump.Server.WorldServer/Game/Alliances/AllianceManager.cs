...
namespace Stump.Server.WorldServer.Game.Alliances
{
    public class AllianceManager : DataManager<AllianceManager>, ISaveable
    {
        ...
        private Dictionary<int, Database.Guilds.EmblemRecord> m_emblems;
        ...
        public void Save()
        {
            lock (m_lock)
            {
                ...
                using (var enumerator = (from alliance in m_alliances.Values where alliance.IsDirty select alliance).ToList().GetEnumerator())
                {
                ...
                while (m_alliancesToDelete.ToList().Count > 0)
                {
        ...
        {
            m_emblems = Database.Query<Database.Guilds.EmblemRecord>(Server.WorldServer.Database.Guilds.EmblemRelator.FetchQuery).ToDictionary(x => x.IconId);
            m_alliances = (from temp in Database.Query<AllianceRecord>(AllianceRelator.FetchQuery).ToList() select new Alliance(temp)).ToDictionary(entry => entry.Id);
            ...
            {
                m_idProvider = new UniqueIdProvider((from temp in m_alliances select temp.Value.Id).Max());
            }

            Singleton<World>.Instance.RegisterSaveableInstance(this);
        }

        public SocialGroupCreationResultEnum CreateAlliance(Character character, string allianceName, string allianceTag, SocialEmblem emblem)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        var basePlayerItem = character.Inventory.TryGetItem(Singleton<ItemManager>.Instance.TryGetTemplate(ALLIOGEMME_ID));
                        ...
                        else
                        {
                            ...
                            character.SendInformationMessage(TextInformationTypeEnum.TEXT_INFORMATION_MESSAGE, 22, 1, basePlayerItem.Template.Id);
                            ...
                            else
                            {
                                ...
                                else
                                {
                                    ...
                                    var document = new BsonDocument
                                    {
                                        ...
                                        { "emblembackgroundColor", emblem.backgroundColor },
                                        { "emblembackgroundShape", emblem.backgroundShape },
                                        { "emblemsymbolColor", emblem.symbolColor },
                                        { "emblemsymbolShape", emblem.symbolShape },
                                        { "Date", DateTime.Now.ToString(CultureInfo.CreateSpecificCulture("pt-BR")) }
                                    };

                                    MongoLogger.Instance.Insert("CreatAlliance", document);
        ...
        public Database.Guilds.EmblemRecord TryGetEmblem(int id)
        {
            Database.Guilds.EmblemRecord record;
        ...
        {
            return m_alliances.Any(pair => string.Equals(pair.Value.Name, name, StringComparison.CurrentCultureIgnoreCase));
        }
        ...
        public bool DoesEmblemExist(SocialEmblem emblem)
        {
            return m_alliances.Any(entry => entry.Value.Emblem.DoesEmblemMatch(emblem));
        }

        public IEnumerable<AllianceFactSheetInformation> GetAlliancesFactSheetInformations()
        {
            lock (m_lock)
            {
                return m_alliances.Values.Select(entry => entry.GetAllianceFactSheetInformations());
            }
        }
    }
}