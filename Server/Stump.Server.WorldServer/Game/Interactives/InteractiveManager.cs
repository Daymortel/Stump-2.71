...
namespace Stump.Server.WorldServer.Game.Interactives
{
    public class InteractiveManager : DataManager<InteractiveManager>
    {
        ...
        public override void Initialize()
        {
            ...
            if (m_interactivesTemplates != null)
                m_interactivesTemplates.Clear();

            if (m_interactivesSpawns != null)
                m_interactivesSpawns.Clear();

            if (m_skillsTemplates != null)
                m_skillsTemplates.Clear();

            if (m_interactivesCustomSkills != null)
                m_interactivesCustomSkills.Clear();

            #endregion

            m_interactivesTemplates = Database.Query<InteractiveTemplate, InteractiveSkillTemplate, InteractiveTemplateSkills, InteractiveCustomSkillRecord, InteractiveTemplate>(new InteractiveTemplateRelator().Map, InteractiveTemplateRelator.FetchQuery).ToDictionary(entry => entry.Id);
            m_interactivesSpawns = Database.Query<InteractiveSpawn, InteractiveSpawnSkills, InteractiveCustomSkillRecord, InteractiveSpawn>(new InteractiveSpawnRelator().Map, InteractiveSpawnRelator.FetchQuery).ToDictionary(entry => entry.Id);
            ...
            m_interactivesCustomSkills = Database.Query<InteractiveCustomSkillRecord>(InteractiveSkillRelator.FetchQuery).ToDictionary(entry => entry.Id);
        ...
        void GenerateInteractivePlot()
        {
            foreach (var trigger in CellTriggerManager.Instance.GetCellTriggers())
            {
                ...
                var interactive = m_interactivesSpawns.Values.FirstOrDefault(x => x.MapId == trigger.MapId && x.CellId == trigger.CellId && x.TemplateId == null && !x.CustomSkills.Any());
        ...
        {
            spawn.ElementId = spawn.Id;
            Database.Insert(spawn);
            Database.Insert(skill);
            spawnSkill.SkillId = skill.Id;
        ...
        public InteractiveSkillTemplate GetDefaultSkillTemplate() => m_defaultSkillTemplate ?? (m_defaultSkillTemplate = GetSkillTemplate(DEFAULT_SKILL_TEMPLATE));
    }
}