...
namespace Stump.Server.WorldServer.Game.Accounts
{
    public class AccountManager : DataManager<AccountManager>
    {
        ...
        public UserGroup GetGroupOrDefault(int id)
        {
            ...
            {
                try { logger.Error("Erro para definir o usergroup do userid: " + id + ", erro:" + ex.Message); }
                catch (Exception exx) { logger.Error("Erro para definir o usergroup do userid: " + exx.Message + ", erro:" + ex.Message);
