...
namespace Stump.Server.WorldServer.Game.Guilds
{
    public class GuildManager : DataManager<GuildManager>, ISaveable
    {
        ...
        ObjectValidator<BasicGuildInformations[]> m_cachedGuildsVersatile;
        ...
        {
            m_emblems = Database.Query<EmblemRecord>(EmblemRelator.FetchQuery).ToDictionary(x => x.IconId);
            m_guildsMembers = Database.Fetch<GuildMemberRecord, CharacterRecord, GuildMemberRecord>(new GuildMemberRelator().Map, GuildMemberRelator.FetchQuery).ToDictionary(x => x.CharacterId, x => new GuildMember(x));

            var membersByGuilds = m_guildsMembers.Values.GroupBy(x => x.Record.GuildId).ToDictionary(x => x.Key);

            m_guilds = Database.Query<GuildRecord>(GuildRelator.FetchQuery).Select(x => new Guild(x, membersByGuilds.ContainsKey(x.Id) ? membersByGuilds[x.Id] : Enumerable.Empty<GuildMember>())).ToDictionary(x => x.Id);
            ...
            m_cachedGuildsVersatile = new ObjectValidator<BasicGuildInformations[]>(BuildCachedGuildsVersatile, TimeSpan.FromMinutes(15));
        ...
        public SocialGroupCreationResultEnum CreateGuild(Character character, string name, NetworkGuildEmblem emblem)
        {
            ...
            bool matched = Regex.IsMatch(name, "^\\b[A-Z][A-Za-z\\s-']{4,30}\\b$", RegexOptions.Compiled);
            bool notmatched = Regex.IsMatch(name, "^\\s\\s$");
            ...
            character.Inventory.RemoveItem(character.Inventory.TryGetItem(ItemIdEnum.GUILDALOGEMME_1575));
            ...
            MongoLogger.Instance.Insert("CreatGuild", document);
        ...
        private BasicGuildInformations[] BuildCachedGuildsVersatile() => m_guilds.Values.Select(x => x.GetGuildVersatileInformations()).ToArray();

        public GuildInformations[] GetCachedGuilds() => m_cachedGuilds;

        public BasicGuildInformations[] GetCachedGuildsVersatile() => m_cachedGuildsVersatile;
        ...
        public void Save()
        {
            ...
            lock (m_lock)
            {
                ...
                while (m_guildsToDelete.Count > 0)
                {
                    ...
                    MongoLogger.Instance.Insert("GuildDelete", document);
                }
                ...
                while (m_membersToDelete.Count > 0)
                {
                    ...
                    MongoLogger.Instance.Insert("MembersGuildDelete", document);
