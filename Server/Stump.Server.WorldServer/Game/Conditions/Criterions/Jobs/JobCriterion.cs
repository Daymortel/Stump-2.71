...
namespace Stump.Server.WorldServer.Game.Conditions.Criterions
{
    public class JobCriterion : Criterion
    {
        ...
        public const string Identifier1 = "Pj";
        ...
        public override bool Eval(Character character)
        {
            ...
            {
                return character.Jobs.Any(entry => entry.Id == Id && entry.Level >= Level);
            }
        ...
        public override void Build()
        {
            ...
            if (Literal.Contains(","))
            {
                ...
                if (split.Length != 2 || !int.TryParse(split[0], out id) || !int.TryParse(split[1], out level))
                    throw new Exception(string.Format("Cannot build JobCriterion, {0} is not a valid job (format 'id,level')", Literal));
            }
            else if (!int.TryParse(Literal, out id))
            {
