...
namespace Stump.Server.WorldServer.Game.Actors.RolePlay.Merchants
{
    public class MerchantManager : DataManager<MerchantManager>, ISaveable
    {
        ...
        public List<MerchantSpawn> FindMerchantByMapCellId(long MapId, int CellId)
        {
            return Database.Fetch<MerchantSpawn>($"SELECT * FROM world_maps_merchant WHERE MapId = {MapId} AND Cell = {CellId}");
        }
