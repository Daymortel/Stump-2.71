...
namespace Stump.Server.WorldServer.Game.Actors.RolePlay.Npcs
{
    public class NpcManager : DataManager<NpcManager>
    {
        ...
        public override void Initialize()
        {
            ...
            if (m_npcsSpawns != null)
                m_npcsSpawns.Clear();

            if (m_npcsTemplates != null)
                m_npcsTemplates.Clear();

            if (m_npcsActions != null)
                m_npcsActions.Clear();

            if (m_npcsReplies != null)
                m_npcsReplies.Clear();

            if (m_npcsMessages != null)
                m_npcsMessages.Clear();
        ...
        {
            return m_npcsTemplates.Values.FirstOrDefault(entry => entry.Name.Equals(name, ignorecase ? StringComparison.InvariantCultureIgnoreCase : StringComparison.InvariantCulture));
        }
        ...
        {
            return m_npcsReplies.Where(entry => entry.Value.NpcId == id).Select(entry => entry.Value).ToList();
        }
