...
namespace Stump.Server.WorldServer.Game.Actors.RolePlay.Characters
{
    public class CharacterManager : DataManager<CharacterManager>
    {
        ...
        [Variable(true)] public static uint MaxVipCharacterSlot = 10;

        private static readonly Regex m_nameCheckerRegex = new Regex("^[A-Z][a-z]{2,9}(?:-[A-Za-z][a-z]{2,9}|[a-z]{1,10})$", RegexOptions.Compiled);
        ...
        {
            WorldServer.Instance.IOTaskPool.EnsureContext();
            List<CharacterRecord> result = Database.Query<CharacterRecord>(string.Format("SELECT * FROM characters WHERE AccountId = {0}", client.Account.Id)).ToList();
            //List<CharacterRecord> list = (
            //    from x in Database.Fetch<CharacterRecord>("SELECT * FROM characters")
            //    where x.AccountId == client.Account.Id
            //    select x).ToList();
        ...
        public void CreateCharacter(WorldClient client, string name, sbyte breedId, bool sex, IEnumerable<int> colors, int headId, Action successCallback, Action<CharacterCreationResultEnum> failCallback)
        {
            ...
            if (client.Characters.Count(x => !x.IsDeleted) >= MaxVipCharacterSlot && client.UserGroup.Role >= RoleEnum.Vip && client.UserGroup.Role <= RoleEnum.Gold_Vip)
            {
                failCallback(CharacterCreationResultEnum.ERR_TOO_MANY_CHARACTERS);
                return;
            }
            ...
            var spellsToLearnVariant = from variant in breed.Spells
                                       where variant.VariantLevel <= breed.StartLevel
                                       orderby variant.VariantLevel, variant.VariantId ascending
                                       select variant;
            var slot = 0;

            foreach (var spellRecord in spellsToLearn.Select(learnableSpell => SpellManager.Instance.CreateSpellRecord(record, SpellManager.Instance.GetSpellTemplate(learnableSpell.Spell))))
            {
                spellRecord.Selected = true;
            ...
            foreach (var spellRecord in spellsToLearnVariant.Select(learnableSpell => SpellManager.Instance.CreateSpellRecord(record, SpellManager.Instance.GetSpellTemplate(learnableSpell.VariantId))))
            {
                Database.Insert(spellRecord);
            }
            ...
            //    if (itemRecord.Template.Id == 15118)
            //    {
            //        var shortcut = new ItemShortcut(record, slot_itens, itemRecord.ItemId, itemRecord.Id);
            //        Database.Insert(shortcut);
            //        slot_itens++;
            //    }
            //}
            ...
            IPCAccessor.Instance.SendRequest(new AddCharacterMessage(client.Account.Id, record.Id), x => successCallback(), x => { Database.Delete(record); failCallback(CharacterCreationResultEnum.ERR_NO_REASON); }); ;
