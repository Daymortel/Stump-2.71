...
namespace Stump.Server.WorldServer.Game.Actors.RolePlay.Monsters
{
    public class MonsterGroup : RolePlayActor, IContextDependant, IAutoMovedEntity
    {
        ...
        public void FightWith(Character character)
        {
            ...
            Map.Leave(this);
            int i = 0;
            this.Map.UpdateFightPlacements((byte)CountInitialFighters());

            while (Map.GetBlueFightPlacement().Length < CountInitialFighters() && i < 10)
            {
                this.Map.UpdateFightPlacements((byte)CountInitialFighters());
                i++;
            }

            if (i >= 10 || Map.GetBlueFightPlacement().Length < CountInitialFighters())
            {
                switch (character.Account.Lang)
                {
                    case "fr":
                        character.SendServerMessage("Ne peut pas démarrer le combat: pas assez d'emplacements de combat");
                        break;
                    case "es":
                        character.SendServerMessage("No se puede iniciar la pelea: No hay suficientes ubicaciones de lucha");
                        break;
                    case "en":
                        character.SendServerMessage("Cannot start fight : Not enough fight placements");
                        break;
                    default:
                        character.SendServerMessage("Não é possível começar a luta: não há canais de luta suficientes");
                        break;
                }
                return;
            }
            ...
            CharacterFighter fighter = character.CreateFighter(fight.ChallengersTeam);
            fight.ChallengersTeam.AddFighter(fighter);

            /*if (character.Inventory.GetItems(DofusProtocol.Enums.CharacterInventoryPositionEnum.INVENTORY_POSITION_COMPANION).Count() != 0)
            {
               fight.ChallengersTeam.AddFighter((character.CreateCompanion(fight.ChallengersTeam)) as FightActor);
            }   ANOTHER IN FIGHT TEAM */
        ...
        public virtual void AddMonster(Monster monster)
        {
            ...
            ////Sistema de Full Star criado para Spawns do mapa Evento(STAFF) by:Kenshin
            //if (Map.SubArea.Id == 501) //Subarea Ilha Eventos
            //{
            //    if ((monster.Group.AgeBonus + 200) <= 200)
            //        monster.Group.AgeBonus += 200;
            //    else if (monster.Group.AgeBonus < 200)
            //        monster.Group.AgeBonus += (short)(200 - monster.Group.AgeBonus);
            //}
        ...
        {
            GameRolePlayGroupMonsterInformations _rolePlayGroup = new GameRolePlayGroupMonsterInformations(
                contextualId: Id,
                look: Leader.Look.GetEntityLook(),
                disposition: GetEntityDispositionInformations(),
                staticInfos: this.GetGroupMonsterStaticInformations(character),
                lootShare: 0,
                alignmentSide: (sbyte)character.AlignmentSide,
                hasHardcoreDrop: false);

            return _rolePlayGroup;
        }

        public virtual GroupMonsterStaticInformations GetGroupMonsterStaticInformations(Character character) => new GroupMonsterStaticInformations(Leader.GetMonsterInGroupLightInformations(), GetMonstersWithoutLeader().Select(entry => entry.GetMonsterInGroupInformations()));
