...
namespace Stump.Server.WorldServer.Game.Actors.Fight
{
    public sealed class CharacterFighter : NamedFighter
    {
        ...
        public void OnTurnStarted(IFight fight, FightActor actor)
        {
            ...
            {
                EffectManager.Instance.GetSpellEffectHandler((EffectDice)effect, this, new DefaultSpellCastHandler(new SpellCastInformations(this, null, Cell)), Cell, false).Apply();
            }
        }
        ...
        public override bool CastSpell(SpellCastInformations cast)
        {
            ...
            if (cast.Spell.Id != 0 || Character.Inventory.TryGetItem(CharacterInventoryPositionEnum.ACCESSORY_POSITION_WEAPON) == null)
            {
                ...
                {
                    logger.Error($"Error casting spell for the fighter character! Error: {ex.Message}, Cast: {cast}, Caster: {Character?.Name}");
                }
            }
            ...
            if (weaponTemplate is null)
            {
                OnSpellCastFailed(cast);
                return false;
            }

            if (!CanUseWeapon(cast.TargetedCell, weaponTemplate))
            {
                OnSpellCastFailed(cast);
                return false;
            }
            ...
            using (Fight.StartSequence(SequenceTypeEnum.SEQUENCE_WEAPON))
            {
                ...
                var effects = weapon.Effects.Where(entry => EffectManager.Instance.IsUnRandomableWeaponEffect(entry.EffectId)).OfType<EffectDice>();
        ...
        public override FightTeamMemberInformations GetFightTeamMemberInformations() => new FightTeamMemberCharacterInformations(Id, Name, (byte)Character.Level);
