...
namespace Stump.Server.WorldServer.Game.Maps
{
    public class SubArea
    {
        ...
        public bool HasPrism { get; set; }
        public PrismNpc Prism { get; internal set; }
        ...
        List<int> MonstersPaskawk = new List<int> { 5731, 5732, 5733, 5734, 5735, 5737, 5738, 5739, 5740, 5741, 5742, 5743, 5744, 5745, 5746, 5747, 5748, 5736, 5757, 5756, 5758, 5759 };
        ...
        public int GetMonsterSpawnsSubAreaCount(int monsterId)
        {
            return GetMaps().SelectMany(entry => entry.GetActors<MonsterGroup>()).Count(group => group.GetMonsters().Any(monster => monster.Template.Id == monsterId));
        }

        public void AddMonsterSpawn(MonsterSpawn spawn)
        {
            var mapsSpawns = Maps.Where(map => map.Outdoor && !map.Record.SpawnDisabled).ToList();

            AsyncRandom random = new AsyncRandom();
            var shuffledMaps = mapsSpawns.OrderBy(item => random.Next()).ToList();

            m_monsterSpawns.Add(spawn);

            foreach (var map in shuffledMaps)
            {
                if (MonstersPaskawk.Contains(spawn.Template.Id) && CanPaskwak())
                    continue;
        ...
        public void OnActorEnter(Character character)
        {
            Enter?.Invoke(character);
        }

        public event Action<Character> Exit;

        public void OnActorExit(Character character)
        {
            Exit?.Invoke(character);
        }

        private bool CanPaskwak()
        {
            if (DateTime.Now >= Settings.StartPaskwak && DateTime.Now <= Settings.EndPaskwak)
                return false;
