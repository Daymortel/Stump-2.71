...
namespace Stump.Server.WorldServer.Game.Maps.Cells
{
    ...
    public class MapPoint
    {
        ...
        public Cell[] getCellsAdjacentsZone(Map map, byte min, byte max)
        {
            var lozenge = new Lozenge(min, max);
            var cells = lozenge.GetCells(map.GetCell(this.CellId),map.GetCell(this.CellId), map).Where(entry => entry.Walkable).ToArray();
