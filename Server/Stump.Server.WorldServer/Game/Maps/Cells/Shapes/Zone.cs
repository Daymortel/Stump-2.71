...
namespace Stump.Server.WorldServer.Game.Maps.Cells.Shapes
{
    public class Zone : IShape
    {
        ...
        public Zone(SpellShapeEnum shape, byte radius, byte minRadius, DirectionsEnum direction, int efficiencyMalus, int maxEfficiency, 
            uint isZoneStopAtTarget = 0, bool isWeapon = false, int casterInfoCellId = 0)
        {
            Radius = radius;
            MinRadius = minRadius;
            Direction = direction;
            ...
            EfficiencyMalus = efficiencyMalus > 0 ? efficiencyMalus : EFFECTSHAPE_DEFAULT_EFFICIENCY;
            MaxEfficiency = maxEfficiency > 0 ? maxEfficiency : EFFECTSHAPE_DEFAULT_MAX_EFFICIENCY_APPLY;
            IsZoneStopAtTarget = isZoneStopAtTarget;
            IsWeapon = isWeapon;
            CasterInfoCellId = casterInfoCellId;
        }
        
        private uint IsZoneStopAtTarget { get; set; }
        private bool IsWeapon { get; set; }
        private int CasterInfoCellId { get; set; } 
        ...
        public Cell[] GetCells(Cell centerCell,Cell casterCell, Map map) => m_shape.GetCells(centerCell, casterCell, map);
        ...
        private void InitializeShape()
        {
            switch (ShapeType)
            {
                ...
                case SpellShapeEnum.L:
                    m_shape = new Line(MinRadius, Radius);
                    break;
                case SpellShapeEnum.l:
                    m_shape = new Line(MinRadius , Radius,  true, IsZoneStopAtTarget == 1);
                    break;
                ...
                case SpellShapeEnum.plus:
                    m_shape = new Cross(0, Radius > 0 ? Radius : (byte)1)
                    {
                        Diagonal = true
                    };
                    break;
                ...
                case SpellShapeEnum.F:
                    m_shape = new Fork(Radius);
                    break;
                case SpellShapeEnum.R:
                    m_shape = new Rectangle(MinRadius, Radius);
                    break;
