...
namespace Stump.Server.WorldServer.Game.Maps
{
    public class Map : WorldObjectsContext, ICharacterContainer
    {
        ...
        private List<MonsterRaceIdEnum> monstersMountCapture = new List<MonsterRaceIdEnum>()
        {
            MonsterRaceIdEnum.MULDOS_SAUVAGES_155,
            MonsterRaceIdEnum.VOLKORNES_183
        };
        ...
        public void UpdateFightPlacements(byte nbFighters = 0)
        {
            ...
            if (Record.BlueFightCells.Length == 0 || Record.RedFightCells.Length == 0)
            {
                ...
                else if (Cells.Count(x => x.Walkable && !x.FarmCell && !x.NonWalkableDuringFight) > 16)
                {
                    ...
                    var ramdomCell = this.GetRandomWalkableCell(x => !x.FarmCell && !x.NonWalkableDuringFight);
                    var fightcells = zone.GetCells(ramdomCell, ramdomCell, this).Where(x => x.Walkable && !x.NonWalkableDuringRP && !interacCells.Contains(x) && !x.NonWalkableDuringFight);
        ...
        public void UpdateCells()
        {
            ...
            m_freeCells = Cells.Where(entry => CellsInfoProvider.IsCellWalkable(entry.Id) && !entry.FarmCell && !entry.NonWalkableDuringFight).OrderBy(x => middle.ManhattanDistanceTo(x)).ToArray();
        }

        public void UpdateAvailableJobs()
        {
            AvailableJobs = m_interactives.Values.SelectMany(x => x.GetSkills()).OfType<SkillCraft>().Select(x => JobManager.Instance.GetJobTemplate(x.SkillTemplate.ParentJobId)).ToArray();
        }
        #endregion
        ...
        private readonly Dictionary<long, MapNeighbour> m_clientMapsAround = new Dictionary<long, MapNeighbour>();
        ...
        public List<MountNpc> Montarias { get; private set; }
        ...
        public Map TopNeighbour
        {
            get
            {
                return TopNeighbourId <= long.MaxValue ? m_topNeighbour ?? (m_topNeighbour = World.Instance.GetMap(TopNeighbourId)) : null;
            }
            set
            {
        ...
        public Map BottomNeighbour
        {
            get
            {
                return BottomNeighbourId <= long.MaxValue ? m_bottomNeighbour ?? (m_bottomNeighbour = World.Instance.GetMap(BottomNeighbourId)) : null;
            }
            set
            {
        ...
        public Map LeftNeighbour
        {
            get
            {
                return LeftNeighbourId <= uint.MaxValue ? m_leftNeighbour ?? (m_leftNeighbour = World.Instance.GetMap(LeftNeighbourId)) : null;
            }
            set
            {
        ...
        public Map RightNeighbour
        {
            get
            {
                return RightNeighbourId <= long.MaxValue ? m_rightNeighbour ?? (m_rightNeighbour = World.Instance.GetMap(RightNeighbourId)) : null;
            }
            set
            {
        ...
        {
            get { return Record.MerchantsDisabled; }
        }
        #endregion
        ...
        public Mount SpawnMount(Character spawn)
        {
            ...
            var npc = spawn.EquippedMount;// new Mount(id, spawn);
        ...
        public Watend SpawnWatendNpc(WatendSpawn wantend, NpcTemplate template, ObjectPosition position, ActorLook look)
        {
            if (position.Map != this)
                throw new Exception("Try to spawn a npc on the wrong map");
            ...
            var npc = new Watend(id, template, position, look, wantend);
            //template.OnNpcSpawned(npc);
        ...
        //    var npcs = GetActors<Npc>(entry => entry.CanBeSee(character) && (character.HasNpcQuest(entry.Id) || entry.HasQuest(character)));
        //    GameRolePlayNpcQuestFlag[] quests = (from x in npcs
        //                                         select new GameRolePlayNpcQuestFlag(
        //                                             character.GetQuestsWithNpc(x.Id).Select(y => (ushort)y.Id).ToArray(), x.GetQuests(character).Select(z => (ushort)z))).ToArray();

        //    return new MapNpcsQuestStatusUpdateMessage(Id, npcs.Select(x => x.Id * -1).ToArray(), quests, GetActors<Npc>(entry => entry.CanBeSee(character)).Except(npcs).Select(x => x.Id * -1).ToArray());
        //}
        ...
        public InteractiveObject SpawnInteractive(InteractiveSpawn spawn)
        {
            ...
            {
                if (Zaap != null)
                    logger.Error("Cannot add a second zaap on the map MapId: {0} InteractiveId: {1}", Zaap.Map.Id, interactiveObject.Id);
        ...
        public bool UseInteractiveObject(Character character, int interactiveId, int skillId, int instId)
        {
            ...
            var adjacentCells = interactiveObject.Position.Point.GetAdjacentCells(true).Where(x => x.IsInMap()).Select(x => Cells[x.CellId]).ToArray();
            ...
            if (!skill.CanUse(character) || character.CurrentUsedSkill != null && !(character.CurrentUsedSkill is SkillAnimate))
            {
                InteractiveHandler.SendInteractiveUseErrorMessage(character.Client, interactiveId, skillId);
                return false;
            }

            interactiveObject.InstId = instId;
        ...
        public void AddMonsterCooldown(int monsterId, int cooldown)
        {
            var monster = m_monsterSpawns.FirstOrDefault(x => x.MonsterId == monsterId);

            if (monster != null)
                monster.NextSpawn = DateTime.Now.AddHours(cooldown);
        }
        ...
        public void AddMonsterDungeonSpawn(MonsterDungeonSpawn spawn)
        {
            ...
            var dungeonSpawningPool = m_spawningPools.FirstOrDefault(entry => entry is DungeonSpawningPool) as DungeonSpawningPool;
        ...
        public MonsterGroup GenerateRandomMonsterGroup(int length)
        {
            ...
            for (var i = 0; i < length; i++)
            {
                ...
                var cumulativeFrequency = 0d;
                MonsterGrade monster = null;

                foreach (var spawn in MonsterSpawns)
                {
                    if (spawn.Template.isQuestMonster)
                    {
                        if (group.GetMonsters().Count(x => x.Template.isQuestMonster && x.Template.Id == spawn.Template.Id) > 0)
                            continue;

                        if (spawn.SubArea.GetMonsterSpawnsSubAreaCount(spawn.MonsterId) > 0)
                            continue;

                        if (DateTime.Compare(spawn.NextSpawn, DateTime.Now) > 0)
                            continue;
                        ...
                        if (DateTime.Compare(spawn.NextSpawn, DateTime.Now) < 0)
                        {
                            World.Instance.SendServerMessageGoldVips(
                                $"O Procurado <b>{TextManager.Instance.GetText(monster.Template.NameId, Languages.Portugese)}</b> acabou de nascer na região <b>{TextManager.Instance.GetText(spawn.SubArea.Record.NameId, Languages.Portugese)}</b>.",
                                $"The Wanted <b>{TextManager.Instance.GetText(monster.Template.NameId, Languages.English)}</b> has just been born in the region <b>{TextManager.Instance.GetText(spawn.SubArea.Record.NameId, Languages.English)}</b>.",
                                $"El Wanted <b>{TextManager.Instance.GetText(monster.Template.NameId, Languages.Spanish)}</b> acaba de nacer en la región <b>{TextManager.Instance.GetText(spawn.SubArea.Record.NameId, Languages.Spanish)}</b>.",
                                $"L'espoir <b>{TextManager.Instance.GetText(monster.Template.NameId, Languages.French)}</b> vient de naître dans la région <b>{TextManager.Instance.GetText(spawn.SubArea.Record.NameId, Languages.French)}</b>.",
                                Color.OrangeRed);
                        }
                    ...
                    cumulativeFrequency += spawn.Frequency;

                    if (roll <= cumulativeFrequency)
                    {
                        if (spawn.Template.IsMiniBoss)
                        {
                            if (group.GetMonsters().Count(x => x.Template.IsMiniBoss && x.Template.Id == spawn.Template.Id) > 0)
                                continue;
                        ...
                        if (monstersMountCapture.Contains((MonsterRaceIdEnum)spawn.Template.Race))
                        {
                            if (group.GetMonsters().Count(x => monstersMountCapture.Contains((MonsterRaceIdEnum)x.Template.Race)) > 0)
                                continue;
        ...
        public MonsterGroup GenerateRandomMonsterGroup(MonsterGroup monsterGroup)
        {
            ...
            foreach (var monster in monsterGroup.GetMonsters().Where(x => !x.Template.IsBoss || !x.Template.IsMiniBoss || !x.Template.isQuestMonster))
            {
        ...
        private static bool CheckMonsterAI(MonsterGrade grade)
        {
            ...
            return categories.Any(x => (x & SpellCategory.Damages) != 0 || x.HasFlag(SpellCategory.Healing));
        }
        ...
        public int GetMonsterSpawnsMapCount(int monsterId)
        {
            return GetActors<MonsterGroup>().Count(actor => actor.GetMonsters().All(entry => MonsterSpawns.Any(spawn => spawn.MonsterId == monsterId)));
        }
        ...
        private void MoveRandomlyActors()
        {
            foreach (var actor in Actors.Where(x => x is IAutoMovedEntity && ((IAutoMovedEntity)x).NextMoveDate <= DateTime.Now))
            {
                ...
                var dest = circle.GetCells(actor.Cell, actor.Cell, this).
        ...
        public Cell[] GetBlueFightPlacement() => m_bluePlacement == null || !m_bluePlacement.Where(x => x != null).Any() ? new Cell[0] : m_bluePlacement;

        public Cell[] GetRedFightPlacement() => m_redPlacement == null || !m_redPlacement.Where(x => x != null).Any() ? new Cell[0] : m_redPlacement;
        ...
        void OnEnter(RolePlayActor actor)
        {
            ...
            var prism = actor as PrismNpc;

            if (prism != null)
            {
                if (Prism != null)
                {
                    logger.Error("There is already a Prism on that map ({0}).", Id);
                    Leave(prism);
                    return;
                }
            ...
            if (actor is IAutoMovedEntity)
            {
                ...
                if (m_autoMoveTimer == null)
                    m_autoMoveTimer = Area.CallPeriodically((AutoMoveActorMaxInverval + AutoMoveActorMinInverval) / 20 * 1000, MoveRandomlyActors);
            }

            actor.OnEnterMap(this);

            if (actor.HasChangedSubArea() && character != null)
            {
                SubArea.OnActorEnter(character);
            }
        ...
        void OnLeave(RolePlayActor actor)
        {
            ...
            if (actor.IsGonnaChangeSubArea() && character != null)
                SubArea.OnActorExit(character);
        }
        ...
        public List<ushort> GetWalkableCell()
        {
            var cells = Cells.Where(x => x.Walkable);

            return cells.Shuffle().ToList().ConvertAll<ushort>(x => (ushort)x.Id);
        }
        ...
        public void ActivateInteractiveObjectForAllPlayer(int interactiveId)
        {
            ...
            foreach (var _character in interactive.Map.GetAllCharacters())
            {
                if (!interactive.CharacterCanSee.Contains(_character))
                    interactive.CharacterCanSee.Add(_character);

                InteractiveHandler.SendInteractiveElementUpdatedMessage(_character.Client, _character, interactive);
            }
        }
        ...
        public void DisableActivateStateInteractiveObjectForAllPlayer(int interactiveId)
        {
            var interactive = GetInteractiveObject(interactiveId);
        ...
        public bool IsDungeon() => (SpawningPools.Any(x => x is DungeonSpawningPool) || MonsterManager.Instance.GetMonsterDungeonsSpawns().Where(spawn => spawn.Map == this || spawn.TeleportMap == this).Any());
        ...
        public MapNeighbour GetClientMapRelativePosition(long mapid)
        {
        ...
        public MapComplementaryInformationsDataMessage GetMapComplementaryInformationsDataMessage(Character character)
        {
            ...
            {
                return CreateMapComplementaryInformationsInHavenBag(character);
            }
            else if (character.SubArea.Id == 916)
            {
                return CreateMapComplementaryInformationsAnomalyMessage(character);
            }
            else
            {
                return CreateMapComplementaryInformationsWithCoords(character);
            }
        }

        private MapComplementaryInformationsWithCoordsMessage CreateMapComplementaryInformationsWithCoords(Character character)
        {
            for (int retryCount = 0; retryCount < 3; retryCount++)
            {
                try
                {
                    List<RolePlayActor> actorsCopy = m_actors;
                    var actorsList = actorsCopy.Where(entry => entry.CanBeSee(character)).Select(entry => entry.GetGameContextActorInformations(character) as GameRolePlayActorInformations).ToList();
                    var interactives = m_interactives.Where(entry => entry.Value.CanBeSee(character));
                    var animatedInteractives = interactives.Where(x => x.Value.Animated);
                    var fightsInformations = m_fights.Where(entry => entry.BladesVisible).Select(entry => entry.GetFightCommonInformations()).ToList();
                    var fightStartPositions = new FightStartingPositions(GetBlueFightPlacement().Select(x => (ushort)x.Id).ToList(), GetRedFightPlacement().Select(x => (ushort)x.Id).ToList());

                    var complementaryInformations = new MapComplementaryInformationsWithCoordsMessage(
                        subAreaId: (ushort)SubArea.Id,
                        mapId: Id,
                        houses: new List<HouseInformations>(),
                        actors: actorsList,
                        interactiveElements: interactives.Select(entry => entry.Value.GetInteractiveElementWithBonus(character)).ToList(),
                        statedElements: animatedInteractives.Select(entry => entry.Value.GetStatedElement()).ToList(),
                        obstacles: GetMapObstacles(),
                        fights: fightsInformations,
                ...
                {
                    if (retryCount == 3)
                        logger.Error($"Error CreateMapComplementaryInformationsWithCoords: {character.NameClean} in the MapId: {character.Map.Id} Message: {ex.Message}\nStackTrace: {ex.StackTrace}");
                }
            }
        ...
        private MapComplementaryInformationsAnomalyMessage CreateMapComplementaryInformationsAnomalyMessage(Character character)
        {
            try
            {
                var actors = m_actors.ToList().Where(entry => entry.CanBeSee(character)).Select(entry => entry.GetGameContextActorInformations(character) as GameRolePlayActorInformations).ToList();
                var interactives = m_interactives.Where(entry => entry.Value.CanBeSee(character)).Select(entry => entry.Value.GetInteractiveElementWithBonus(character));
                var animatedInteractives = m_interactives.Where(entry => entry.Value.CanBeSee(character)).Where(x => x.Value.Animated);
                var fightsInformations = m_fights.Where(entry => entry.BladesVisible).Select(entry => entry.GetFightCommonInformations());
                var fightStartPositions = new FightStartingPositions(GetBlueFightPlacement().Select(x => (ushort)x.Id), GetRedFightPlacement().Select(x => (ushort)x.Id));
                ...
                return new MapComplementaryInformationsAnomalyMessage(
                    ...
                    statedElements: animatedInteractives.Select(entry => entry.Value.GetStatedElement()),
                    obstacles: GetMapObstacles(),
                    fights: fightsInformations,
                    hasAggressiveMonsters: false,
                    fightStartPositions: fightStartPositions,
                    level: AnomalyManager.GetAnomalylevel(),
                    closingTime: AnomalyManager.GetAnomalyClosing());
            }
            catch (Exception ex)
            {
                logger.Error($"Error CreateMapComplementaryInformationsInHavenBag: {character.NameClean} in the MapId: {character.Map.Id} Message: {ex}");
                return CreateMapComplementaryInformationsAnomalyMessage(character);
            }
        }

        private MapComplementaryInformationsDataMessage CreateMapComplementaryInformationsInHavenBag(Character character)
        {
            try
            {
                var actors = m_actors.ToList().Where(entry => entry.CanBeSee(character)).Select(entry => entry.GetGameContextActorInformations(character) as GameRolePlayActorInformations).ToList();
                ...
                return new MapComplementaryInformationsDataInHavenBagMessage(
                    ...
                    hasAggressiveMonsters: false,
                    fightStartPositions: fightStartPositions,
                    ownerInformations: HavenBagManager.Instance.GetHavenBagOwner(character).GetCharacterMinimalInformations(),
                    theme: HavenBagManager.Instance.GetHavenBagTheme(character),
                    roomId: HavenBagManager.Instance.GetHavenBagRoomId(character),
                    maxRoomId: HavenBagManager.Instance.GetHavenBagMaxRoom(character));
            }
            catch (Exception ex)
            {
                logger.Error($"Error CreateMapComplementaryInformationsInHavenBag: {character.NameClean} in the MapId: {character.Map.Id} Message: {ex}");
        ...
        private MapComplementaryInformationsDataInHouseMessage CreateMapComplementaryInformationsDataInHouseMessage(Character character)
        {
            List<RolePlayActor> actorsCopy = m_actors;
            ...
            var complementaryInformations = new MapComplementaryInformationsDataInHouseMessage(
                subAreaId: 0,
                mapId: Id,
                houses: new List<HouseInformations>(),
                actors: actorsList,
