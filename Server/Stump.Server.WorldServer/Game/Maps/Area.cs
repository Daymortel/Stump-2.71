...
namespace Stump.Server.WorldServer.Game.Maps
{
    public class Area : IContextHandler
    {
        ...
        [Variable]
        public static readonly int CheckDCTime = 20;
        ...
        public void CheckDC()
        {
            try
            {
                var charactersToRemove = m_characters.Where(x => !x.IsLoggedIn).ToList();
                ...
                foreach (var character in charactersToRemove)
                {
                    if (!character.IsLoggedIn)
                    {
                        ...
                        m_objects.RemoveAll(obj => obj is Character && !((Character)obj).IsLoggedIn);

                        //logger.Warn($"Character " + character.NameClean + " disconnected. Last Activity: " + DateTime.Now.Subtract(character.Client.LastActivity).TotalMinutes + " minutes removed from " + character.Area.Name);
                    }
                }
            }
            catch (Exception ex)
            {
                logger.Error("An error occurred in CheckDC: {0}", ex);
                throw;
            }
        }
