...
namespace Stump.Server.WorldServer.Game.Maps.Spawns
{
    public class StaticSpawningPool : SpawningPoolBase
    {
        ...
        public StaticSpawningPool(Map map) : this(map, StaticSpawnsInterval)
        {
        }
        ...
        private void OnWinnersDetermined(IFight fight, FightTeam winners, FightTeam losers, bool draw)
        {
            ...
            if (spawn.TeleportMap.Id == 0)
                return;

            foreach (var fighter in winners.GetAllFighters<CharacterFighter>())
            {
                var celltotp = spawn.TeleportCell.Id != 0 ? spawn.TeleportCell : fighter.Cell;
                var directiontotp = spawn.TeleportDirection != null ? (DirectionsEnum)spawn.TeleportDirection : fighter.Direction;
                fighter.Character.NextMap = spawn.TeleportMap;
