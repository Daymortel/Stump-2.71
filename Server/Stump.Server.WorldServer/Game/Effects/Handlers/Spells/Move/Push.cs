...
namespace Stump.Server.WorldServer.Game.Effects.Handlers.Spells.Move
{
    ...
    {
        public Push(EffectDice effect, FightActor caster, SpellCastHandler castHandler, Cell targetedCell, bool critical) : base(effect, caster, castHandler, targetedCell, critical)
        {
            DamagesDisabled = effect.EffectId == EffectsEnum.Effect_PushBack_1103 || effect.EffectId == EffectsEnum.Effect_PullForward;
            Pull = effect.EffectId == EffectsEnum.Effect_PullForward;
        }

        public bool DamagesDisabled { get; set; }

        public DirectionsEnum? PushDirection { get; set; }
        ...
        protected override bool InternalApply()
        {
            ...
            //Spell Escapatoria de SRAM - Correção Versão 2.61.10
            if (Spell.Id == (int)SpellIdEnum.EVASION_14746)
            {
                actors.Add(Caster.LastAttacker);
            }
            ...
            foreach (var actor in actors)
            {
                ...
                if (Spell.Id == (int)SpellIdEnum.ODYSSEY_5390)
                {
                ...
                //Spell Fricção de IOP - Correção Versão 2.61.10
                if (Spell.Id == (int)SpellIdEnum.FRIKT_13128)
                {
                    referenceCell = actor.LastAttacker.Cell;
                }

                //if (Spell.Id == (int)SpellIdEnum.OUTPOURING_13126) //Version 2.61.10
                //    referenceCell = actor.LastAttacker.Cell;
                ...
                for (var i = 0; i < Distance; i++)
                {
                    ...
                    if (IsBlockingCell(nextCell, actor) || (pushDirection.IsDiagonal() && pushDirection.GetDiagonalDecomposition().Any(x => IsBlockingCell(lastCell.GetNearestCellInDirection(x), actor))))
                    {
                        if (nextCell == null)
                        {
                            stopCell = lastCell;
                        ...
                        {
                            #region >> PORTALS
                            var PortalSpellOne = new Spell((int)SpellIdEnum.SUCCESSFUL_TELEPORTAL_14597, 1);
                            var PortalSpellTwo = new Spell((int)SpellIdEnum.SUCCESSFUL_TELEPORTAL_5426, 1);
                            ...
                            if (portal != null)
                            {
                                if (portal.IsActive && Fight.PortalsManager.GetActivableGamePortalsCount(portal.Caster.Team) >= 2)
                                {
                                    ...
                                    if (links.Count > 0)
                                    {
                                        ...
                                        ActionsHandler.SendGameActionFightTeleportOnSameMapMessage(Fight.Clients, Caster, actor, finalPortalCell);

                                        actor.Position.Cell = finalPortalCell;

                                        var spellCastHandlerOne = SpellManager.Instance.GetSpellCastHandler(Fight.FighterPlaying, PortalSpellOne, actor.Cell, false);

                                        spellCastHandlerOne.MarkTrigger = portal;
                                        spellCastHandlerOne.Initialize();
                                        spellCastHandlerOne.Execute();

                                        var spellCastHandlerTwo = SpellManager.Instance.GetSpellCastHandler(Fight.FighterPlaying, PortalSpellTwo, actor.Cell, false);

                                        spellCastHandlerTwo.MarkTrigger = portal;
                                        spellCastHandlerTwo.Initialize();
                                        spellCastHandlerTwo.Execute();
