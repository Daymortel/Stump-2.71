...
namespace Stump.Server.WorldServer.Game.Effects.Handlers.Spells.Move
{
    ...
    {
        private int[] Spells_Ignore_Point =
        {

        };

        private int[] Spells_Ignore_Target =
        {
            (int)SpellIdEnum.PROJECTION_9958,
            (int)SpellIdEnum.PROJECTION_12726,
            (int)SpellIdEnum.EXODUS_9387,
            (int)SpellIdEnum.EXODUS_14616,
            (int)SpellIdEnum.PENDULUM_9901,
            (int)SpellIdEnum.TELEPORTATION_88,
            (int)SpellIdEnum.HUMERUS_13794,
            (int)SpellIdEnum.TELEPORTATION_88,
            (int)SpellIdEnum.TELEPORTATION_7767,
            (int)SpellIdEnum.TELEPORTATION_5427,
            (int)SpellIdEnum.TELEPORTATION_13263,
            (int)SpellIdEnum.TELEPORTATION_13249,
            (int)SpellIdEnum.PENDULUM_13294,
            (int)SpellIdEnum.GAMBOL_14015,
            (int)SpellIdEnum.WHIP_12557,
        };
            ...
            foreach (var target in GetAffectedActors())
            {
                ...
                switch (Effect.EffectId)
                {
                    ...
                    case EffectsEnum.Effect_SymetricPointTeleport:
                        if (!Spells_Ignore_Point.Contains(Spell.Template.Id))
                        {
                            casterPoint = target.Position.Point;
                    ...
                    case EffectsEnum.Effect_SymetricTargetTeleport:
                        if (!Spells_Ignore_Target.Contains(Spell.Template.Id))
                        {
                            casterPoint = target.Position.Point;
                            targetPoint = TargetedPoint;
                        }
                ...
                if (fighter != null && fighter != target)
                {
                    ...
                    else if (Effect.EffectId == EffectsEnum.Effect_SymetricPointTeleport && Spell.Id == (int)SpellIdEnum.GAMBOL_14015)
                    {
                        caster.Telefrag(Caster, fighter);
                        TelefraggedActor.Add(fighter);
                    }
                ...
                else
                {
                    ...
                    if (!Spells_Ignore_Target.Contains(Spell.Template.Id) || !Spells_Ignore_Point.Contains(Spell.Template.Id))
                    {
                        if (Effect.EffectId == EffectsEnum.Effect_SymetricCasterTeleport || Effect.EffectId == EffectsEnum.Effect_SymetricPointTeleport)
                            caster = target;
                    }
    ...
    {
        public Recurse(EffectDice effect, FightActor caster, SpellCastHandler castHandler, Cell targetedCell, bool critical) : base(effect, caster, castHandler, targetedCell, critical)
        { }
