...
namespace Stump.Server.WorldServer.Game.Effects.Handlers.Spells.Heal
{
    ...
    {
        public GiveHpWhenAttack(EffectDice effect, FightActor caster, SpellCastHandler castHandler, Cell targetedCell, bool critical) : base(effect, caster, castHandler, targetedCell, critical)
        {
    ...
    public class GiveHpPercentWhenAttack : SpellEffectHandler
    {
        ...
        protected override bool InternalApply()
        {
            foreach (var actor in GetAffectedActors())
            {
                ...
                if (Spell.Id == (int)SpellIdEnum.BAGRIFICE_13351 || Spell.Id == (int)SpellIdEnum.SHARING_13373)
