...
namespace Stump.Server.WorldServer.Game.Effects.Handlers.Spells
{
    public abstract class SpellEffectHandler : EffectHandler
    {
        ...
        [Variable]
        public static bool _logCast = false;

        protected SpellEffectHandler(EffectDice effect, FightActor caster, SpellCastHandler castHandler, Cell targetedCell, bool critical) : base(effect)
        {
            Dice = effect;
            Caster = caster;
            ...
            if (_logCast)
                logger.Debug(
                    "Handle '{0}' effect for the spell '{1}', for the mask {2}, to the dice {3}, to the face {4}, the target {5}, the dice Value {6}, the delay {7}, the duration {8}, the shape {9}.",
                    effect.EffectId, castHandler.Spell.Template, effect.TargetMask, effect.DiceNum, effect.DiceFace, Targets,
                    Dice.Value,
                    Effect.Delay, Effect.Duration, Effect.ZoneShape);
        }
        ...
        {
            get { return m_affectedCells ?? (m_affectedCells = EffectZone.GetCells(TargetedCell, CastCell, Map).ToList()); }
            /*private */
            set { m_affectedCells = value; }
        }
        ...
        {
            if (actor == Caster && _logCast)
            {
                var casterStates = Caster.GetStates();
                logger.Debug($"The caster {Caster} has states: {string.Join(",", casterStates)}");
            }
            ...
            return lookup.All(x => x.First().IsDisjonction ? x.Any(y => y.IsTargetValid(actor, this)) : x.All(y => y.IsTargetValid(actor, this)));
        }

        public void RefreshZone()
        {
            AffectedCells = EffectZone.GetCells(TargetedCell, CastCell, Map).ToList();
        }
        ...
        public IEnumerable<FightActor> GetAffectedActors()
        {
            ...
            var fighters = Fight.GetAllFighters(AffectedCells).Where(entry => !entry.IsDead() && !entry.IsCarried() && IsValidTarget(entry)).ToArray();
        ...
        public void SetAffectedActor(FightActor actor)
        {
            m_customAffectedActors = new FightActor[] { actor };
        }
        ...
        public bool HasTargetEffect(FightActor target) => target.Buffs.Any(x => x.Spell.Template.Id == Effect.SpellId);

        public Buff AddStatBuff(FightActor target, short value, PlayerFields caracteritic, short? customActionId = null)
        {
            if (IsTriggerBuff() && !HasTargetEffect(target))
        ...
        protected Buff AddStateBuffDirectly(FightActor target, bool bypassMaxStack, SpellState state, FightActor triggerer = null)
        {
            if (target is null)
            {
                logger.Error($"AddStateBuffDirectly Error: target is null. Parameters - bypassMaxStack: {bypassMaxStack}, state: {state}, triggerer: {(triggerer != null ? triggerer.Owner.Name : "null")}");
                return null;
            }
