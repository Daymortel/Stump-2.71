...
namespace Stump.Server.WorldServer.Game.Effects.Handlers.Spells.Armor
{
    ...
    {
        public GiveShieldPercent(EffectDice effect, FightActor caster, SpellCastHandler castHandler, Cell targetedCell, bool critical) : base(effect, caster, castHandler, targetedCell, critical)
        { }
        ...
        protected override bool InternalApply()
        {
            foreach (var actor in GetAffectedActors())
            {
                ...
                {
                    if (Spell.Id == (int)SpellIdEnum.EMERALD_DOFUS_8393)
                    {
                        var buff = Caster.GetBuffs(entry => entry.Spell.Id == (int)SpellIdEnum.EMERALD_DOFUS_8393 && entry.Dice.Id == 1039).ToList();
