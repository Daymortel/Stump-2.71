        ...
        void ReviveActor(FightActor actor, int heal)
        {
            ...
            if (Spell.Id == (int)SpellIdEnum.SPIRITUAL_LEASH_420)
            {
                try
                {
                    ...
                    var addStateHandler = new AddState(new EffectDice(EffectsEnum.Effect_AddState, (short)SpellStatesEnum.ZOMBIE_74, 0, 0), actor, null, actor.Cell, false);
                    var actorBuff = new StateBuff(actorBuffId, actor, Caster, addStateHandler,
                        Spell, FightDispellableEnum.DISPELLABLE_BY_DEATH, SpellManager.Instance.GetSpellState((uint)SpellStatesEnum.ZOMBIE_74))
                    {
                        Duration = -1000
                    };
