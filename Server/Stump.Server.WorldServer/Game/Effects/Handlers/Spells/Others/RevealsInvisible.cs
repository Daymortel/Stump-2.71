...
namespace Stump.Server.WorldServer.Game.Effects.Handlers.Spells.Others
{
    ...
    public class RevealsInvisible : SpellEffectHandler
    {
        ...
        {
            var containedTraps = Fight.GetTriggers().OfType<Trap>().Where(entry => entry.VisibleState == GameActionFightInvisibilityStateEnum.INVISIBLE && Caster.IsEnnemyWith(entry.Caster) && AffectedCells.Contains(entry.Shape.Cell));
