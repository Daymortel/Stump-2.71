...
namespace Stump.Server.WorldServer.Game.Effects
{
    public class EffectManager : DataManager<EffectManager>
    {
        ...
        {
            { CharacteristicEnum.ACTION_POINTS_AP, 101 },
            { CharacteristicEnum.MOVEMENT_POINTS_MP, 90 },
            { CharacteristicEnum.RANGE, 51 },
            { CharacteristicEnum.REFLECT, 30 },
            { CharacteristicEnum.SUMMONS, 30 },
            { CharacteristicEnum.CRITICAL, 10 },
        ...
        {
            {CharacteristicEnum.ACTION_POINTS_AP, 1},
            {CharacteristicEnum.MOVEMENT_POINTS_MP, 1},
            {CharacteristicEnum.RANGE, 1},
            {CharacteristicEnum.REFLECT, 1},
            {CharacteristicEnum.SUMMONS, 1},
            {CharacteristicEnum.CRITICAL, 2},
        ...
        public override void Initialize()
        {
            ...
            if (m_itemsEffectHandler != null)
                m_itemsEffectHandler.Clear();

            if (m_itemsSetEffectHandler != null)
                m_itemsSetEffectHandler.Clear();

            if (m_usablesEffectHandler != null)
                m_usablesEffectHandler.Clear();

            if (m_spellsEffectHandler != null)
                m_spellsEffectHandler.Clear();

            if (m_effectsHandlers != null)
                m_effectsHandlers.Clear();
        ...
        public SpellEffectHandler GetSpellEffectHandler(EffectDice effect, FightActor caster, SpellCastHandler castHandler, Cell targetedCell, bool critical)
        {
            ...
            SpellEffectHandler RefNull = null;

            try
            {
                RefNull = m_spellsEffectHandler.TryGetValue(effect.EffectId, out handler) ? handler(effect, caster, castHandler, targetedCell, critical) : new DefaultSpellEffect(effect, caster, castHandler, targetedCell, critical);
            }
            catch
            {
                logger.Error("Effect:" + effect + "EffectID:" + effect.EffectId + " Caster name:" + (caster as CharacterFighter).Character);
            }
