...
namespace Stump.Server.WorldServer.Game.Exchanges.Craft.Runes
{
    ...
    {
        public SingleRuneMagicCraftDialog(Character character, InteractiveObject interactive, Skill skill) : base(interactive, skill, character.Jobs[skill.SkillTemplate.ParentJobId])
        {
        ...
        protected override void OnRuneApplied(CraftResultEnum result, MagicPoolStatus poolStatus)
        {
            ...
            InventoryHandler.SendExchangeCraftInformationObjectMessage(Character.Client, ItemToImprove.PlayerItem, (ExchangeCraftResultEnum)result);

            Character.Map.Clients.Send(new GameRolePlayDelayedObjectUseMessage(Character.Id, (sbyte)DelayedActionTypeEnum.DELAYED_ACTION_OBJECT_USE, timecraft, (ushort)ItemToImprove.PlayerItem.Template.Id));
            Character.Map.Clients.Send(new GameRolePlayDelayedActionFinishedMessage(Character.Id, (sbyte)DelayedActionTypeEnum.DELAYED_ACTION_OBJECT_USE));
