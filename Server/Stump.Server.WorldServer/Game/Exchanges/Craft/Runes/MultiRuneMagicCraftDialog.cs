...
namespace Stump.Server.WorldServer.Game.Exchanges.Craft.Runes
{
    ...
    {
        public MultiRuneMagicCraftDialog(Character crafter, Character customer, InteractiveObject interactive, Skill skill) : base(interactive, skill, crafter.Jobs[skill.SkillTemplate.ParentJobId])
        {
        ...
        {
            InventoryHandler.SendExchangeCraftPaymentModifiedMessage(Crafter.Character.Client, (long)kamasamount);
            InventoryHandler.SendExchangeCraftPaymentModifiedMessage(Receiver.Character.Client, (long)kamasamount);
        }
        ...
        protected override void OnRuneApplied(CraftResultEnum result, MagicPoolStatus poolStatus)
        {
            ...
            InventoryHandler.SendExchangeCraftInformationObjectMessage(Crafter.Character.Client, ItemToImprove.PlayerItem, (ExchangeCraftResultEnum)result);
            InventoryHandler.SendExchangeCraftInformationObjectMessage(Receiver.Character.Client, ItemToImprove.PlayerItem, (ExchangeCraftResultEnum)result);

            Crafter.Character.Map.Clients.Send(new GameRolePlayDelayedObjectUseMessage(Crafter.Character.Id, (sbyte)DelayedActionTypeEnum.DELAYED_ACTION_OBJECT_USE, timecraft, (ushort)ItemToImprove.PlayerItem.Template.Id));
