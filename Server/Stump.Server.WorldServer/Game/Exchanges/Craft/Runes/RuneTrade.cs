...
namespace Stump.Server.WorldServer.Game.Exchanges.Craft.Runes
{
    public class RuneTrade : ITrade
    {
        ...
        public void Decraft()
        {
            ...
            InventoryHandler.SendDecraftResultMessage(Character.Client, results.Select(x => new DecraftedItemStackInfo((uint)x.Key.Guid, (float)(x.Value.MinCoeff ?? 0.5), (float)(x.Value.MaxCoeff ?? 0.5), x.Value.Runes.Select(y => (uint)y.Key.Id).ToArray(), x.Value.Runes.Select(y => (uint)y.Value).ToArray())));
