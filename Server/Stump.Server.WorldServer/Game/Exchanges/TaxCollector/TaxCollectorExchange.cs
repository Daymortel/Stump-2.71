...
namespace Stump.Server.WorldServer.Game.Exchanges.TaxCollector
{
    public class TaxCollectorExchange : IExchange
    {
        ...
        public DofusProtocol.Messages.ExchangeTaxCollectorGetMessage GetExchangeGuildTaxCollector()
        {
            return new DofusProtocol.Messages.ExchangeTaxCollectorGetMessage($"" +
                $"{TaxCollector.FirstNameId.ToBase(36)},{TaxCollector.LastNameId.ToBase(36)}",
                (short)TaxCollector.Position.Map.Position.X,
                (short)TaxCollector.Position.Map.Position.Y,
                TaxCollector.Position.Map.Id,
                (ushort)TaxCollector.Position.Map.SubArea.Id,
                Character.Name,
                (ulong)TaxCollector.Record.CallerId,
                TaxCollector.Record.CallerName,
                (ushort)m_collector.RecoltedItems.Values.Sum(x => x.Template.RealWeight * x.Stack),
                m_collector.RecoltedItems.Values.Select(x => x.GetObjectItemGenericQuantity()),
                TaxCollector.Look.GetEntityLook());
        }
    }
}