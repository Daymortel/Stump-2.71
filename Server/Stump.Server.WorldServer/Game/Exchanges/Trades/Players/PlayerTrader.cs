    ...
    public class PlayerTrader : Trader
    {
        ...
        public virtual bool MoveItemToInventory(TradeItem item, int quantity)
        {
            ...
            else
            {
                ...
                if (item is PlayerTradeItem) {
                    if ((item as PlayerTradeItem).Owner == this.Character)
                        OnItemMoved(item, true, quantity);
                    else
                        OnItemMoved(item, true, quantity);
                }
        ...
        public virtual bool MoveItemToInventory_Receiver(int id, int quantity)
        {
            var item = Items.FirstOrDefault(x => x.Guid == id);
        ...
        {
            var traderune = Character.Dialog as RuneTrade;
            ...
            if (item.Effects.Exists(x => x.EffectId == EffectsEnum.Effect_BlockItemNpcShop) && Character.Account.UserGroupId <= 3 && traderune == null)
            {
                BasicHandler.SendTextInformationMessage(Character.Client, TextInformationTypeEnum.TEXT_INFORMATION_ERROR, 345, item.Template.Id, item.Guid);
                return false;
            }
