...
namespace Stump.Server.WorldServer.Game.Parties
{
    public class Party
    {
        ...
        protected virtual void OnGuestPromoted(Character groupMember)
        {
            ...
            ArenaGroupLevelSum += groupMember.Level > 200 ? 200 : groupMember.Level;
        ...
        private void OnContextChanged(Character character, bool infight)
        {
            ...
            if (character.Area.Id == 68)
            {
                ...
                {
                    if (character.Fighter == null)//test
                        return;
                    if (character.Fighter.Team == null)//test
                        return;
                    if (character.Fighter.Team.Leader == null)//test
                        return;
                    ...
                    {
                        PartyHandler.SendPartyMemberInBreachFightMessage(clients, this, character, character.Fighter.Team == character.Fight.ChallengersTeam ? PartyFightReasonEnum.ATTACK_PLAYER : PartyFightReasonEnum.PLAYER_ATTACK, character.Fight);
                    }
            ...
            else
            {
                character.Area.AddMessage(() =>
                {
                    ...
                    if (character.Fight is FightAgression)
                    {
                        PartyHandler.SendPartyMemberInStandardFightMessage(clients, this, character, character.Fighter.Team == character.Fight.ChallengersTeam ? PartyFightReasonEnum.ATTACK_PLAYER : PartyFightReasonEnum.PLAYER_ATTACK, character.Fight);
                    }
                    else if (character.Fight is FightPvM)
                    {
