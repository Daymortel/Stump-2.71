...
{
    public abstract class AbstractCriterion<T, Data> : AbstractCriterion where T : AbstractCriterion<T, Data> where Data : CriterionData
    {
        // FIELDS
        ...
        public override void Parse(AchievementObjectiveRecord objective)
        {
            ...
            foreach (var item in criterions)
            {
                ...
                if (match.Success)
                {
                    m_criterions.Add(Parse(GetOperator(match.Groups["operator"].Value[0]), match.Groups["content"].Value.Split(new[] { ',' }, StringSplitOptions.RemoveEmptyEntries)));
                }
                else
                {
                    Console.WriteLine(">> Error AbstractCriterion: " + item);
                    throw new Exception();
                }
            }
        }
    ...
    public abstract class AbstractCriterion
    {
        ...
        public static ComparaisonOperatorEnum GetOperator(char c)
        {
            ...
            switch (c)
            {
                ...
                case '=':
                    ...
                    break;
                case '!':
                    result = ComparaisonOperatorEnum.INEQUALS;
                    break;
        ...
        public static AbstractCriterion CreateCriterion(AchievementObjectiveRecord objective)
        {
            ...
            if (!string.IsNullOrWhiteSpace(objective.Criterion) && objective.Criterion.Length >= 2)
            {
                ...
                {
                    var name = objective.Criterion;
                    var charsToRemove = new string[] { "(", ")" };
                    name = charsToRemove.Aggregate(name, (current, c) => current.Replace(c, string.Empty));
                    ...
                    switch (name)
                    {
                        ...
                        case UnknowCriterion.Identifier:
                            result = new UnknowCriterion(objective);
                            break;
                        ...
                        case KillBossCriterion.Identifier:
                            ...
                            if (bossCriterion.Monster != null)
                                Singleton<AchievementManager>.Instance.AddKillBossCriterion(bossCriterion);
                            break;
                        ...
                        case KillBossWithChallengeCriterion.Identifier:
                            ...
                            if (bossChallCriterion.Monster != null)
                                Singleton<AchievementManager>.Instance.AddKillBossWithChallengeCriterion(bossChallCriterion);
                            break;
                        ...
                        case QuestFinishedCriterion.Identifier:
                            ...
                            Singleton<AchievementManager>.Instance.AddQuestFinishedCriterion(questFinishedCriterion);
                            break;
                        ...
                        case HaveItemCriterion.Identifier:
                            ...
                            Singleton<AchievementManager>.Instance.AddhaveItemCriterion(haveItemCriterion);
                            break;
