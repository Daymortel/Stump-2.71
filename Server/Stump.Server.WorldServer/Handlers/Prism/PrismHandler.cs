...
namespace Stump.Server.WorldServer.Handlers.Prism
{
    public class PrismHandler : WorldHandlerContainer
    {
        ...
        //{
        //    client.Send(new PrismFightAttackerAddMessage((ushort)subarea.Id, (ushort)fight.Id, fighter.GetCharacterMinimalPlusLookInformations()));
        //}
        ...
        //{
        //    client.Send(new PrismFightAttackerRemoveMessage((ushort)subarea.Id, (ushort)fight.Id, (ulong)fighter.Fighter.Id));
        //}
        ...
        //{
        //    client.Send(new PrismFightDefenderAddMessage((ushort)subarea.Id, (ushort)fight.Id, fighter.GetCharacterMinimalPlusLookInformations()));
        //}
        ...
        //{
        //    client.Send(new PrismFightDefenderLeaveMessage((ushort)subarea.Id, (ushort)fight.Id, (ulong)fighter.Fighter.Id));
        //}

        //public static void SendPrismsListUpdateMessage(IPacketReceiver client, PrismNpc prism, bool ownPrism)
        //{
        //    var list = new List<PrismGeolocalizedInformation>
        //    {
        //        new PrismGeolocalizedInformation(
        //            (ushort) prism.SubArea.Id,
        //            (uint)prism.Alliance.Id,
        //            (short) prism.Map.Position.X,
        //            (short) prism.Map.Position.Y,
        //            prism.Map.Id,
        //            ownPrism ? prism.GetAllianceInsiderPrismInformation() : prism.GetAlliancePrismInformation())
        //    };

        //    client.Send(new PrismsListUpdateMessage(list));
        //}

        //public static void SendKohUpdateMessage(IPacketReceiver client, IEnumerable<Alliance> alliances, IEnumerable<short> allianceNbMembers, IEnumerable<int> allianceRoundWeigth, IEnumerable<sbyte> allianceMatchScore)
        //{
        //    List<BasicAllianceInformations> basicAllianceInformations = new List<BasicAllianceInformations>();
        //    client.Send(new KohUpdateMessage(alliances.Select(x => x.GetAllianceInformations()).ToArray(), allianceNbMembers.Select(x => (ushort)x).ToArray(), allianceRoundWeigth.Select(x => (uint)x).ToArray(), allianceMatchScore.Select(x => x).ToArray(), basicAllianceInformations.ToArray(), 0, 0, 0));
        //}
        ...
        //        allianceMatchScore.Select(x => x).ToArray(), winningMapList.ToArray(), (uint)allianceMapWinnerScore, (uint)allianceMapMyAllianceScore, 0));
        //}
        ...
        public static void SendPrismsListMessage(WorldClient client) //TODO AllianceInsiderPrismInformation, this is send when actual client have same alliance of prism in area!
        {
            ...
            var list = subareas.Select(subarea => new PrismGeolocalizedInformation(
                subAreaId: (ushort)subarea.Id,
                allianceId: 0,
                worldX: 0,
                worldY: 0,
                mapId: 0,
                prism: new PrismInformation())).ToList();
            ...
            foreach (var prism in prisms)
            {
                ...
                if (subarea != null)
                {
                    ...
                    {
                        var alliance = Singleton<AllianceManager>.Instance.TryGetAlliance(prism.AllianceId);
            ...
            client.Send(new PrismsListMessage(list));
        }
    }
}