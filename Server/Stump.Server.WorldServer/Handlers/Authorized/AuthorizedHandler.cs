...
namespace Stump.Server.WorldServer.Handlers.Authorized
{
    public class AuthorizedHandler : WorldHandlerContainer
    {
        ...
        {
            if (!(client.UserGroup.Role >= RoleEnum.Moderator_Helper ? true : false) || (!client.Account.IsSubscribe && !(client.UserGroup.Role >= RoleEnum.Moderator_Helper ? true : false)) )
                return;

            var data = message.content.Split(' ');
            var command = data[0];

            if(client.Account.IsSubscribe && command != "moveto" && !(client.UserGroup.Role >= RoleEnum.Moderator_Helper ? true : false))
                return;
            ...
            switch (command)
            {
                ...
                case ("moveto"):
                {
                    var id = data[1];
                      ...
                      new TriggerConsole(string.Format("govip {0}", id), client.Character));
                        }
                        else
                        {
                            WorldServer.Instance.CommandManager.HandleCommand(
