...
namespace Stump.Server.WorldServer.Handlers.Context
{
    public partial class ContextHandler
    {
        ...
        public static void HandleGameActionFightCastRequestMessage(WorldClient client, GameActionFightCastRequestMessage message)
        {
            ...
            {
                var spell = client.Character.Companion.GetSpell(message.spellId);
            ...
            {
                var spell = fighter.IsSlaveTurn() ? fighter.GetSlave().GetSpell(message.spellId) : fighter.GetSpell(message.spellId);
        ...
        public static void HandleGameFightReadyMessage(WorldClient client, GameFightReadyMessage message)
        {
            ...
            if (client.Character.Fight is FightPvT)
                return;
            if (client.Character.Fight is FightPvMr)
                return;
        ...
        public static void HandleGameRolePlayPlayerFightRequestMessage(WorldClient client, GameRolePlayPlayerFightRequestMessage message)
        {
            ...
            //Bloqueando qualquer tipo de luta no mapa evento se o evento pega pega estiver ativo
            if (Settings.PegaPega && client.Character.Map.SubArea.Id == 84 && !client.Character.IsGameMaster())
                return;

            if (!message.friendly && client.Character.SubArea.HasPrism && client.Character.AvAActived)
            {
                if (client.Character.SubArea.Prism.State == PrismStateEnum.PRISM_STATE_VULNERABLE)
                {
                    var actor = client.Character.Map.GetActor<Character>((int)message.targetId);

                    if (actor != null)
                    {
                        var fighterRefusedReasonEnum = client.Character.CanAgressAvA(actor);
                        if (fighterRefusedReasonEnum != FighterRefusedReasonEnum.FIGHTER_ACCEPTED)
                        {
                            SendChallengeFightJoinRefusedMessage(client, client.Character, fighterRefusedReasonEnum);
                            return;
                        }
                        else
                        {
                            actor.StopMove();
                            var fight = Singleton<FightManager>.Instance.CreateAgressionFightKoh(actor.Map);
                ...
                {
                    CreateFightPVP(client, target); //Sistema novo de PVP by: Kenshin
                }
            }
            else if (message.friendly)
            {
                //Criando a luta do Evento Pega-Pega
                if (Settings.PegaPega && client.Character.Map.SubArea.Id == 84 && client.Character.IsGameMaster())
                {
                    var fight = FightManager.Instance.CreateDuel(client.Character.Map);
                    fight.DefendersTeam.AddFighter(client.Character.CreateFighter(fight.DefendersTeam));
        ...
        private static void CreateFightPVP(WorldClient client, Character target)
        {
            ...
            else if (Settings.ModoPVP == false)
            {
                #region MSG
                switch (client.Character.Account.Lang)
                {
                    case "fr":
                        client.Character.SendServerMessage("Vous n'êtes pas autorisé à entrer dans un combat. Ce serveur n'a pas le système PVP publié.", System.Drawing.Color.Red);
                        break;
                    case "es":
                        client.Character.SendServerMessage("No se le permite participar en una pelea. Este servidor no tiene el sistema PVP liberado.", System.Drawing.Color.Red);
                        break;
                    case "en":
                        client.Character.SendServerMessage("You are not allowed to enter a fight. This server does not have the PVP system released.", System.Drawing.Color.Red);
                        break;
                    default:
                        client.Character.SendServerMessage("Você não tem permissão para entrar em uma luta. Esse servidor não possui o sistema de PVP liberado.", System.Drawing.Color.Red);
                        break;
                }
                #endregion
            }
            else
            {
                ...
                var fight = FightManager.Instance.CreateAgressionFight(target.Map, client.Character.AlignmentSide, target.AlignmentSide);
        ...
        public static void HandleGameFightJoinRequestMessage(WorldClient client, GameFightJoinRequestMessage message)
        {
            ...
            if (fight is FightPvD && message.fighterId != 0)
            {
                SendChallengeFightJoinRefusedMessage(client, client.Character, FighterRefusedReasonEnum.FIGHTER_REFUSED);
                return;
            }
            ...
            //Evento Pega-Pega by:Kenshin
            if (Settings.PegaPega && client.Character.Map.SubArea.Id == 84)
            {
                SendChallengeFightJoinRefusedMessage(client, client.Character, FighterRefusedReasonEnum.TEAM_LIMITED_BY_MAINCHARACTER);
                return;
            }
            ...
            //Restrição Staff Hydra para Staff entrar na luta By:Kenshin
            CharacterFighter fighterPlayerRefused = (CharacterFighter)fight.ChallengersTeam.Fighters.Where(x => x is CharacterFighter).FirstOrDefault();

            if (client.UserGroup.Role >= RoleEnum.Moderator_Helper && client.UserGroup.Role <= RoleEnum.GameMaster && fighterPlayerRefused.Character.Client.UserGroup.Role <= RoleEnum.Gold_Vip)
            {
                #region MSG
                switch (client.Character.Account.Lang)
                {
                    case "fr":
                        client.Character.SendServerMessage("Vous n'êtes pas autorisé à participer à un combat d'état-major.", System.Drawing.Color.Red);
                        break;
                    case "es":
                        client.Character.SendServerMessage("No se le permite participar en una pelea de personal.", System.Drawing.Color.Red);
                        break;
                    case "en":
                        client.Character.SendServerMessage("You are not allowed to enter a Staff fight.", System.Drawing.Color.Red);
                        break;
                    default:
                        client.Character.SendServerMessage("Você não tem permissão para entrar em uma luta de Staff.", System.Drawing.Color.Red);
                        break;
                }
                #endregion
            ...
            //Restrição Staff Hydra para Player entrar na luta By:Kenshin
            CharacterFighter fighterStaffRefused = (CharacterFighter)fight.ChallengersTeam.Fighters.Where(x => x is CharacterFighter).FirstOrDefault();

            if (client.UserGroup.Role >= RoleEnum.Gold_Vip && fighterStaffRefused.Character.Client.UserGroup.Role >= RoleEnum.Moderator_Helper && fighterStaffRefused.Character.Client.UserGroup.Role <= RoleEnum.GameMaster)
            {
                #region MSG
                switch (client.Character.Account.Lang)
                {
                    case "fr":
                        client.Character.SendServerMessage("Vous n'êtes pas autorisé à entrer dans un combat. Vérifiez vos droits avec STAFF", System.Drawing.Color.Red);
                        break;
                    case "es":
                        client.Character.SendServerMessage("No se le permite participar en una pelea. Verifique sus derechos con STAFF", System.Drawing.Color.Red);
                        break;
                    case "en":
                        client.Character.SendServerMessage("You are not allowed to enter a fight. Check your rights with STAFF", System.Drawing.Color.Red);
                        break;
                    default:
                        client.Character.SendServerMessage("Você não tem permissão para entrar em uma luta. Consulte seus direitos com a STAFF", System.Drawing.Color.Red);
                        break;
                }
                #endregion
        ...
        public static void SendGameFightStartingMessage(IPacketReceiver client, FightTypeEnum fightTypeEnum, int attackerId, int defenderId, IFight fight) //TODO - 2.66
        {
            var containBoss = fight.Fighters.Any(fightActor => (fightActor is MonsterFighter fighter) && fighter.Monster.Template.IsBoss);
            var monstersList = fight.Fighters.Where(fightActor => fightActor is MonsterFighter fighter).Select(entry => entry.Id);

            GameFightStartingMessage _startingFight = new GameFightStartingMessage()
            {
                fightType = (sbyte)fightTypeEnum,
                fightId = (ushort)fight.Id,
                attackerId = attackerId,
                defenderId = defenderId,
                containsBoss = containBoss,
                monsters = monstersList
            };

            client.Send(_startingFight);
        }
        ...
        public static void SendGameFightJoinMessage(IPacketReceiver client, bool canBeCancelled, bool canSayReady, bool isFightStarted, int timeMaxBeforeFightStart, FightTypeEnum fightTypeEnum)
        {
            client.Send(new GameFightJoinMessage(!isFightStarted, canBeCancelled, canSayReady, isFightStarted, (short)timeMaxBeforeFightStart, (sbyte)fightTypeEnum));
        }

        public static void SendGameFightSpectatorJoinMessage(IPacketReceiver client, IFight fight)
        {
            client.Send(new GameFightSpectatorJoinMessage(false, false, false, fight.IsStarted, fight.IsStarted ? (short)0 : (short)(fight.GetPlacementTimeLeft().TotalMilliseconds / 100), (sbyte)fight.FightType, fight.GetPartiesName().ToArray()));
        }
        ...
        public static void SendGameFightSpectateMessage(IPacketReceiver client, IFight fight)
        {
            client.Send(new GameFightSpectateMessage(
                ...
                !fight.IsStarted ? 0 : fight.StartTime.GetUnixTimeStamp(),
                Array.Empty<GameFightEffectTriggerCount>()));
        }
        ...
        {
            client.Send(new GameFightSynchronizeMessage(fight.GetAllFighters().Select(entry => entry.GetGameFightFighterInformations(client)).ToArray()));
        }
        ...
        {
            client.Send(new GameFightUpdateTeamMessage((ushort)fight.Id, team.GetFightTeamInformations()));
        }
        ...
        public static void SendSlaveSwitchContextMessage(IPacketReceiver client, SummonedFighter actor)
        {
            ...
            client.Send(new SlaveSwitchContextMessage(actor.Summoner.Id, actor.Id, (ushort)actor.TurnTime,
                actor.Spells.Select(x => x.Value.GetSpellItem()).ToArray(),
                actor.GetSlaveCharacteristicsInformations(),
                actor.Spells.Select(x => new ShortcutSpell(slotIndex++, (ushort)x.Value.Template.Id)).ToArray()));
        }
        ...
        public static void SendSlaveSwitchContextMessage(IPacketReceiver client, CompanionActor actor)
        {
            ...
            client.Send(new SlaveSwitchContextMessage(actor.Master.Id, actor.Id, (ushort)actor.TurnTime,
                actor.CompanionSpell.Select(x => x.GetSpellItem()).ToArray(),
                actor.GetCompanionStatsMessage(),
                actor.CompanionSpell.Select(x => new ShortcutSpell(slotIndex++, (ushort)x.Template.Id)).ToArray()));
        }
        ...
        public static void SendGameFightResumeMessage(IPacketReceiver client, CharacterFighter fighter)
        {
            try
            {
                if (fighter == null)
                {
                    Console.WriteLine("Error: 'fighter' is null.");
                    return;
                }

                var slaves = fighter.Summons?.Where(x => x != null && x.IsControlled());
