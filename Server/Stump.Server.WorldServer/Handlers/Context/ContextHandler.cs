...
namespace Stump.Server.WorldServer.Handlers.Context
{
    public partial class ContextHandler
    {
        ...
        {
            if (sub)
                client.Send(new GameActionFightDispellableEffectMessage((ushort)ActionsEnum.ACTION_DEBOOST_SPELL_RANGE, buff.Caster.Id, buff.GetAbstractFightDispellableEffect()));
            else
                client.Send(new GameActionFightDispellableEffectMessage((ushort)ActionsEnum.ACTION_BOOST_SPELL_RANGE, buff.Caster.Id, buff.GetAbstractFightDispellableEffect()));
        }
        ...
        public static void HandleGameContextCreateRequestMessage(WorldClient client, GameContextCreateRequestMessage message)
        {
            if (client.Character.IsInWorld)
            {
                ...
                {
                    case "fr":
                        client.Character.SendServerMessage("Vous êtes déjà connecté!");
                        break;

                    case "es":
                        client.Character.SendServerMessage("¡Ya has iniciado sesión!");
                        break;
                    ...
                    default:
                        client.Character.SendServerMessage("Você já logou!");
                        break;
                }
        ...
        public static void HandleGameMapMovementRequestMessage(WorldClient client, GameMapMovementRequestMessage message)
        {
            ...
            else
            {
                ...
                {
                    foreach (var following in client.Character.following)
                    {
                        var pathfinder = new Pathfinder(client.Character.Position.Map.CellsInfoProvider);
                        var excludedCells = movementPath.EndPathPosition.Map.GetActors<RolePlayActor>().Select(entry => entry.Cell.Id);
                        var ceell_id = movementPath.EndPathPosition.Point.GetAdjacentCells(true).Where(x => x.IsInMap()).OrderBy(x => x.ManhattanDistanceTo(movementPath.EndPathPosition.Point)).Where(x => movementPath.EndPathPosition.Map.Cells[x.CellId].Walkable && !excludedCells.Contains(x.CellId)).FirstOrDefault().CellId;
                        var path = pathfinder.FindPath(following.Cell.Id, ceell_id, false);
        ...
        public static void HandleGameMapMovementCancelMessage(WorldClient client, GameMapMovementCancelMessage message)
        {
            ...
            foreach (var following in client.Character.following)
            {
                following.StopMove(new ObjectPosition(following.Map, (short)message.cellId, following.Position.Direction));
            }
        }
        ...
        public static void HandleTeleportHavenBagRequestMessage(WorldClient client, TeleportHavenBagRequestMessage message)
        {
            ...
            if (character.IsInFight() || character.IsBusy())
                return;
