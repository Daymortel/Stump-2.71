...
namespace Stump.Server.WorldServer.Handlers.Titles
{
    public class TitleHandler : WorldHandlerContainer
    {
        ...
        {
            client.Send(new TitlesAndOrnamentsListMessage(character.Titles.Select(x => (ushort)x).ToArray(),
                character.Ornaments.Select(x => (ushort)x).ToArray(),
                (ushort)
                (character.SelectedTitle.HasValue ? character.SelectedTitle.Value : 0),
                (ushort)(character.SelectedOrnament.HasValue ? character.SelectedOrnament.Value : 0)));
        }
        ...
        {
            client.Send(new OrnamentGainedMessage((short)ornament));
        }
    }
}