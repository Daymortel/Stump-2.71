...
namespace Stump.Server.WorldServer.Handlers.TaxCollector
{
    ...
    {
        [WorldHandler(StartListenTaxCollectorUpdatesMessage.Id)]
        public static void HandleGuildFightJoinRequestMessage(WorldClient client, StartListenTaxCollectorUpdatesMessage message)
        {
            ...
            {
                foreach (var a in client.Character.Guild.Alliance.m_guilds.Values)
                {
                    if (a.TaxCollectors.FirstOrDefault(x => x.GlobalId == message.taxCollectorId) != null)
                    {
                        taxCollector = a.TaxCollectors.FirstOrDefault(x => x.GlobalId == message.taxCollectorId);
                    }
                }
            }
        ...
        public static void HandleGameRolePlayTaxCollectorFightRequestMessage(WorldClient client, GameRolePlayTaxCollectorFightRequestMessage message)
        {
            ...
            if (taxCollector != null)
            {
                ...
                if (client.Character.Guild.Alliance != null && taxCollector.Guild.Alliance != null)
                {
                    if (client.Character.Guild.Alliance.Id == taxCollector.Guild.Alliance.Id)
                    {
                        ContextHandler.SendChallengeFightJoinRefusedMessage(client, client.Character, FighterRefusedReasonEnum.WRONG_ALLIANCE);
                        return;
                    }
                }
        ...
        [WorldHandler(StopListenTaxCollectorUpdatesMessage.Id)]
        public static void HandleGuildFightLeaveRequestMessage(WorldClient client, StopListenTaxCollectorUpdatesMessage message)
        {
            if (client.Character.Guild == null)
                return;
        ...
        {
            //client.Send(new TaxCollectorListMessage(guild.TaxCollectors.Select(x => x.GetNetworkTaxCollector()).ToArray(), (sbyte)guild.MaxTaxCollectors, guild.TaxCollectors.Where(x => x.IsFighting).Select(x => x.Fighter.GetTaxCollectorFightersInformation()).ToArray(), 0));
        }

        public static void SendTaxCollectorAttackedMessage(IPacketReceiver client, TaxCollectorNpc taxCollector)
        {
            client.Send(new TaxCollectorAttackedMessage(
                firstNameId: (ushort)taxCollector.FirstNameId,
                lastNameId: (ushort)taxCollector.LastNameId,
                worldX: (short)taxCollector.Map.Position.X,
                worldY: (short)taxCollector.Map.Position.Y,
                mapId: taxCollector.Map.Id,
                subAreaId: (ushort)taxCollector.Map.SubArea.Id,
                alliance: taxCollector.Guild.Alliance.GetBasicAllianceInformations()));
        }
        ...
        {
            client.Send(new TaxCollectorAttackedResultMessage(deadOrAlive, taxCollector.GetTaxCollectorBasicInformations(), taxCollector.Guild.Alliance.GetBasicAllianceInformations()));
        }

        public static void SendGetExchangeGuildTaxCollectorMessage(IPacketReceiver client, ExchangeTaxCollectorGetMessage exchangeGuildTaxCollectorGetMessage)
        {
