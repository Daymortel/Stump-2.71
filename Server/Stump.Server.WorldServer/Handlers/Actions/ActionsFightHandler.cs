...
namespace Stump.Server.WorldServer.Handlers.Actions
{
    public partial class ActionsHandler : WorldHandlerContainer
    {
        ...
        {
            client.Send(new GameActionFightDeathMessage((ushort)ActionsEnum.ACTION_CHARACTER_DEATH, source.Id, fighter.Id));
        }
        ...
        {
            client.Send(new GameActionFightTackledMessage((ushort)ActionsEnum.ACTION_CHARACTER_ACTION_TACKLED, source.Id, tacklers.Select(entry => (double)entry.Id)));
        }

        //Version 2.61 by Kenshin - OK
        public static void SendGameActionFightLifePointsGainMessage(IPacketReceiver client, ActionsEnum action, FightActor source, FightActor target, short delta)
        {
            client.Send(new GameActionFightLifePointsGainMessage((ushort)action, source.Id, target.Id, (uint)delta));

            var characteristics = new List<CharacterCharacteristic>()
            {
                new CharacterCharacteristicDetailed(contextModif: (short)source.Stats.Health.Context, @base: (short)(source.Stats.Health.Base * 1), alignGiftBonus: 0, characteristicId: (short)CharacteristicEnum.HEALTH_POINTS_HP, objectsAndMountBonus: (short)source.Stats.Health.Equiped, additional: (short)source.Stats.Health.Additional),
            };

            client.Send(new RefreshCharacterStatsMessage(source.Id, new GameFightCharacteristics(new CharacterCharacteristics(characteristics), 0, false, (sbyte)(client == null ? source.VisibleState : source.GetVisibleStateFor(source)))));
        }
        ...
        public static void SendGameActionFightPointsVariationMessage(IPacketReceiver client, ActionsEnum action, FightActor source, FightActor target, short delta)
        {
            ...
            var characteristics = new List<CharacterCharacteristic>()
            {
                new CharacterCharacteristicDetailed(0, (short)source.Stats[PlayerFields.AP].Total, 0, (int)CharacteristicEnum.ACTION_POINTS_AP, 0, 0),
                new CharacterCharacteristicDetailed(0, (short)source.Stats[PlayerFields.MP].Total, 0, (int)CharacteristicEnum.MOVEMENT_POINTS_MP, 0, 0),
            };
        ...
        public static void SendGameActionFightLifePointsLostMessage(IPacketReceiver client, ActionsEnum action, FightActor source, FightActor target, short loss, short permanentDamages, int elementId)
        {
            ...
            var characteristics = new List<CharacterCharacteristic>()
            {
                new CharacterCharacteristicDetailed(0, (short)(loss * -1), 0, (int)CharacteristicEnum.SHIELD, 0, 0),
            };

            var invisibilityState = (sbyte)(client == null ? source.VisibleState : source.GetVisibleStateFor(source));

            client.Send(new RefreshCharacterStatsMessage(source.Id, new GameFightCharacteristics(new CharacterCharacteristics(characteristics), 0, false, invisibilityState)));
        }
        ...
        public static void SendGameActionFightLifeAndShieldPointsLostMessage(IPacketReceiver client, ActionsEnum action, FightActor source, FightActor target, short loss, short permanentDamages, int elementId, short shieldLoss)
        {
            ...
            var characteristics = new List<CharacterCharacteristic>()
            {
                new CharacterCharacteristicDetailed(0, shieldLoss, 0, (int)CharacteristicEnum.SHIELD, 0, 0),
            };
        ...
        public static void SendGameActionUpdateSpellModifierMessage(IPacketReceiver client, CharacterSpellModificationTypeEnum modificationType, FightActor actorSource, ushort spellId)
        {
            //var spellModification = new CharacterSpellModification
            //(
            //    modificationType: (sbyte)modificationType, spellId: spellId,
            ...
            //client.Send(new UpdateSpellModifierMessage(actorSource.Id, spellModification));
        }
        ...
        {
            ushort action = duration > 0 ? (ushort)ActionsEnum.ACTION_CHARACTER_ADD_SPELL_COOLDOWN : (ushort)ActionsEnum.ACTION_CHARACTER_REMOVE_SPELL_COOLDOWN;

            client.Send(new GameActionFightSpellCooldownVariationMessage(action, source.Id, target.Id, (ushort)spell.Id, duration));
        }
        ...
        //Version 2.61 by Kenshin - OK
        public static void SendGameActionStatsRefresh(IPacketReceiver client, FightActor source, Boolean isBuff = false)
        {
            if (isBuff)
                client.Send(new RefreshCharacterStatsMessage(source.Id, new GameFightCharacteristics(new CharacterCharacteristics(GetFightActorCharacteristicBuffs(source)), 0, false, (sbyte)(client == null ? source.VisibleState : source.GetVisibleStateFor(source)))));
            else
                client.Send(new RefreshCharacterStatsMessage(source.Id, new GameFightCharacteristics(new CharacterCharacteristics(source.GetFightActorCharacteristic()), 0, false, (sbyte)(client == null ? source.VisibleState : source.GetVisibleStateFor(source)))));
        }

        public static List<CharacterCharacteristic> GetFightActorCharacteristicBuffs(FightActor actor)
        {
            var CharacterList = new List<CharacterCharacteristic>()
            {
                new CharacterCharacteristicDetailed(0, (short)actor.Stats[PlayerFields.AP].Total, 0, (int)CharacteristicEnum.ACTION_POINTS_AP, 0, 0),
                new CharacterCharacteristicDetailed(0, (short)actor.Stats[PlayerFields.MP].Total, 0, (int)CharacteristicEnum.MOVEMENT_POINTS_MP, 0, 0),
                new CharacterCharacteristicDetailed(0, (short)actor.Stats[PlayerFields.Range].Total, 0, (int)CharacteristicEnum.RANGE, 0, 0),
                new CharacterCharacteristicDetailed(0, (short)actor.Stats.Shield.TotalSafe, 0, (int)CharacteristicEnum.SHIELD, 0, 0),
            };
