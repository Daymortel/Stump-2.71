            ...
            if (m_handlers.TryGetValue(message.MessageId, out handlers))
            {
                foreach (var handler in handlers)
                {
                    ...
                    if (context != null)
                    {
                        ...
                        uint lastMessageId = client.GetLastMessageIdOrDefault();

                        if (lastMessageId > 0 && client.AsyncMessageInstance < uint.MaxValue)
                        {
                            client.AsyncMessageInstance++;

                            uint seq = client.AsyncMessageInstance;
                            ushort lastPacketId = (ushort)lastMessageId;

                            client.Send(new BasicAckMessage(seq, lastPacketId));
                        }
                    }
                }
            }
